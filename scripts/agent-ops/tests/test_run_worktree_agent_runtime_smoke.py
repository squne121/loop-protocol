...
def test_given_claude_gpt_default_interactive_lane_then_fixed_policy_channel_reaches_pane(
    monkeypatch, tmp_path
):
    """The non-hook-sink Claude-GPT lane must use an existing fixed channel.

    This captures both environment-generation hops because the launcher's
    settings file is generated by the process Herdr starts in the pane, not by
    this runner process itself.
    """
    module = _load_module()
    ...
    monkeypatch.setattr(module, "new_isolated_session_name", lambda *_args, **_kwargs: "rts-fixture")
    monkeypatch.setattr(module, "create_isolated_session", lambda *_args, **_kwargs: session_process)
    ...
    def fake_run(argv, *, env=None, **_kwargs):
        ...
        if command == ["agent", "get"]:
            return 0, '{"state":"done"}', "", False
        if command == ["agent", "explain"]:
            return 0, '{"agent":"claude","confidence":"high"}', "", False
        return 0, "", "", False

    monkeypatch.setattr(module, "_run", fake_run)
    evidence = {"cleanup": {}}
    module.run_interactive_herdr_isolated(
        "claude", str(tmp_path), "fixture prompt", 10.0, "run-id", evidence,
        herdr_bin="herdr", claude_adapter="claude-gpt", hook_sink_enabled=False,
    )
    ...
    assert ["--env", "CLAUDE_GPT_RUNTIME_SMOKE_HOOKS=subagent-start-stop"] == [
        workspace_argv[index:index + 2]
        for index, value in enumerate(workspace_argv)
        if value == "--env"
        and workspace_argv[index + 1] == "CLAUDE_GPT_RUNTIME_SMOKE_HOOKS=subagent-start-stop"
    ][0]
