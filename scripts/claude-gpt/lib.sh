...
# 内で実装できる範囲は launcher-owned settings のこの追加分に限られる。Claude
# child の GitHub CLI auth は native-equivalent であり（GH_CONFIG_DIR も
# isolation 前の native gh configuration directory のまま使用する）、raw `gh`
# 認証そのものを遮断する仕組みではない。raw `gh` / raw `git push` を
# PreToolUse hook で deterministic に拒否する実装は、project
