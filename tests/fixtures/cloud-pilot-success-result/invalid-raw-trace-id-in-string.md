...
この fixture は自由記述文字列内に生の trace_id が混入した不正を検証するための負例であり、checker が拒否することを確認するためのものである。以下のマーカー行はチェッカーが参照する契約でありバイト単位で変更しない。
...
```json
{
  ...
  "diagnostic_context": {
    "note": "observed correlation abcdef0123456789abcdef0123456789 during run"
  }
}
```

この digest 値は fixture の内容から算出された既存の値であり、意図的な不正フィールドを含んだまま保持している。
<!-- CLOUD_PILOT_SUCCESS_RESULT_DIGEST_V1 sha256=0cec290af8689cc8a3285f463a4e755e7b90af2c5434533829eb4f7aa2ac7b63 -->
