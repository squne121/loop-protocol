...
この fixture は diagnostic_context に未知の nested field を含む負例であり、diagnostic_context が closed object (additionalProperties:false, properties:{}) として拒否することを確認するためのものである（OWNER Blocker 6, fix_delta iteration 2）。以下のマーカー行はチェッカーが参照する契約でありバイト単位で変更しない。
...
```json
{
  ...
  "diagnostic_context": {
    "unexpected_field": "nested value"
  }
}
```

この digest 値はこの fixture の内容（diagnostic_context の未知 nested field を含む）から正しく再計算された fresh digest である。
<!-- CLOUD_PILOT_SUCCESS_RESULT_DIGEST_V1 sha256=d2e06322eadca6827b41e8fdc5396a033a5c709800b1370a82eef925ad650716 -->
