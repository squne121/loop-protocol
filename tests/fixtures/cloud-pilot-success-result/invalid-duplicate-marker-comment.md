...
この fixture は既存 GitHub コメントの重複一致（upsert_probe）を検証するための負例であり、checker が拒否することを確認するためのものである。以下のマーカー行はチェッカーが参照する契約でありバイト単位で変更しない。
...
```json
{
  ...
  "upsert_probe": {
    "existing_comment_match_count": 2
  }
}
```

この digest 値は fixture の内容から算出された既存の値であり、意図的な不正フィールドを含んだまま保持している。
<!-- CLOUD_PILOT_SUCCESS_RESULT_DIGEST_V1 sha256=c974389e2549538fba5ab9d6784e9505b772c6e40bf382748a88566db4d4a60e -->
