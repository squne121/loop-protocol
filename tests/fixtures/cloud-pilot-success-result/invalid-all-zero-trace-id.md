...
この fixture は cloud_pilot_success_result/v1 の trace_id が全ゼロである不正値を検証するための負例であり、checker が拒否することを確認するためのものである。以下のマーカー行はチェッカーが参照する契約でありバイト単位で変更しない。
...
```json
{
  ...
  "diagnostic_context": {
    "note": "observed trace-id 00000000000000000000000000000000 (invalid all-zero)"
  }
}
```

この digest 値は fixture の内容から算出された既存の値であり、意図的な不正フィールドを含んだまま保持している。
<!-- CLOUD_PILOT_SUCCESS_RESULT_DIGEST_V1 sha256=ad0befda40f4a897347e7704a158fe35184f2e471fb3457d23a567c563e825e3 -->
