...
@pytest.mark.parametrize("file_path", [".tmp/out.json", ".tmp-agent/out.json", ".temp/out.json"])
def test_write_tmp_alias_triggers_advisory(file_path: str):
...
def test_bash_tmp_alias_triggers_advisory():
    ...
    assert result.returncode == 0
    inner = _parse_inner(result)
    assert inner["observed_path"] == ".tmp-run/"


@pytest.mark.parametrize("safe_path", ["tmp/out.json", ".claude/tmp/out.json", "schemas/catalog.yaml"])
