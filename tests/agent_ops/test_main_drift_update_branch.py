...
SPEC = importlib.util.spec_from_file_location("update_branch_main_drift", SCRIPT)
...
def test_given_invalid_expected_head_when_update_branch_validates_then_no_poll_or_api_is_authorized():
    request = MODULE.UpdateBranchRequest(
        pr_number=1,
...
def test_given_production_update_branch_when_inspected_then_poll_bounds_are_fixed_and_positive():
    assert MODULE.PRODUCTION_POLL_MAX > 0
    assert MODULE.PRODUCTION_POLL_INTERVAL > 0
