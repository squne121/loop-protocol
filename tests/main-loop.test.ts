...
// AC3: onLoadGame の storage.load() フェーズゲートは tests/hud-controller.test.ts の
// 'Load Game phase policy' describe ブロックにある button.click() + storage spy テストで確認する。
// main.ts の onLoadGame クロージャは非公開のため、ここでは直接テストしない。
