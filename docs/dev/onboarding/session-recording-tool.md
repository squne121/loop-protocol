...
**Conditional の理由**: EntireCLI は `--skip-push-sessions` / private checkpoint / manual review が **必須前提** でのみ候補。
default 採用ではない。
...
- [ ] Schema validation コマンドが存在するか確認
  ...
  # validation script が存在するか（例）
  test -f scripts/validate-manifest.sh || test -c "which jq"
  ```
...
#### EntireCLI を使う場合
```bash
# EntireCLI クライアント起動時に --skip-push-sessions フラグを指定
entire session start --skip-push-sessions

# または設定ファイルで指定
# ~/.entire/config.yaml または .entire/config.yaml に以下を追記
session_recording:
  auto_push_sessions: false
  skip_push_sessions: true
```
...
# または local-only mode で実行
entire session start --checkpoint-local-only
```
...
cat artifacts/session-manifests/agent-session-manifest-*.json | grep -v '"ai_suggested_manifest"'
# マッチしない（含まれていない）のが正常
```
...
```bash
# プロセスを停止
ps aux | grep entire | grep -v grep | awk '{print $2}' | xargs kill -9

# 起動スクリプト / alias 削除
rm -f ~/.entire/auto-start.sh
grep -r "entire session start" ~ 2>/dev/null | grep -v ".history" | sed 's/:.*//g' | xargs sed -i '/entire session start/d'
...
# 機密情報が含まれる場合
find artifacts/ -name "*.log" -o -name "*.json" | xargs shred -vfz -n 3 2>/dev/null || find artifacts/ -name "*.log" -o -name "*.json" | xargs rm -f

# または zip で暗号化保存（後に off-repo で security review）
zip -e -P <secure-password> remaining-transcripts.zip artifacts/
rm -rf artifacts/
...
### 安全設定
- [ ] No-push 設定を確認（EntireCLI: `--skip-push-sessions`, Hook: `MANIFEST_SKIP_PUSH=true`）
...
- `--skip-push-sessions` / `MANIFEST_SKIP_PUSH=true` を **必須**
