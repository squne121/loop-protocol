...
- [ ] **Deterministic producer 実装**
      （`.claude/hooks/post-commit-manifest` / `scripts/generate-session-manifest.sh` /
...
- [ ] **Issue #243** (agent-session-manifest.schema.json) が main に merged 済みか確認
  ...
  # 出力: CLOSED であること
  git ls-remote origin refs/heads/main | xargs git cat-file -p | grep -q "docs/schemas/agent-session-manifest.schema.json"
  ```
...
- [ ] Deterministic producer が存在するか確認（hook / script / CI 等）
  ```bash
  # Hook ベース deployment が存在するか
  test -f .claude/hooks/post-commit-manifest || test -f .claude/hooks/pre-push-manifest
  # または
  # CI script が存在するか
  test -f scripts/generate-session-manifest.sh
  # または
  # GitHub Actions workflow が存在するか
...
- [ ] Checkpoint branch 検証コマンドが成功するか確認
  ```bash
  # 公開 checkpoint branch が誤って存在していないかを確認
  git ls-remote origin refs/heads/entire/checkpoints/v1
  # 出力が空であること（branch が存在しないことを確認）
  ```
...
> **重要な前置き（EntireCLI 例について）**
>
> 以下に出てくる `entire ...` から始まる CLI 例は **概念例** である。
> 実 flag 名・サブコマンド名・設定キー名は本書の作成時点では公式根拠を確認できていない。
> 実行前に **必ず以下を確認** すること:
>
> - `entire --help` および各サブコマンドの `--help`
> - 公式ドキュメント (<https://docs.entire.io/>) のバージョン対応 CLI reference
>
> 本書が示しているのは「**no-push / private checkpoint / local-only / manual review** を
> CLI レベルで強制する」というセキュリティ要件である。
> 等価な flag・設定が存在しない場合は、当該機能を有効化してはならない。

#### EntireCLI を使う場合（概念例）
```bash
# 概念例: session 起動時に「sessions の自動 push を無効化する」flag を指定する
#   実 flag 名は公式 docs で確認すること（例: --skip-push-sessions 等は未検証）
entire session start <no-push-flag>

# あるいは設定ファイルで session 自動 push を無効化する
# ~/.entire/config.yaml または .entire/config.yaml に等価設定を追記
# （キー名は公式 docs を参照）
session_recording:
  auto_push_sessions: false
```
...
#### EntireCLI の場合（概念例）
```bash
# Private checkpoint repository を作成（人間が実行）
# 別途 GitHub private repo を作成し、local machine からのみ push 許可

# 概念例: local-only mode（checkpoint を remote に push しない）で起動する
#   実 flag 名は公式 docs で確認すること（例: --checkpoint-local-only 等は未検証）
entire session start <local-only-flag>
```
...
# producer フィールドが authoritative であることを確認
# （hook_generated / script_generated / github_action_generated / human_attested_from_deterministic_evidence 等）
...
2. **Git hook 無効化** — hook ファイルに実行権限を除去
   ```bash
...
# 注意: 以下の `sed -i` を含むコマンドは home 配下を破壊的に編集する可能性があるため
#       そのまま実行せず、対象ファイルを目視で特定してから手動で消すことを推奨する。
rm -f ~/.entire/auto-start.sh
# 起動コマンドが残っていそうな箇所を「検出のみ」する（編集はしない）
grep -RIn "entire session start" ~ 2>/dev/null | grep -v ".history" || true

# 設定ファイルから session recording 設定を削除
# ~/.entire/config.yaml から session_recording block を削除（手動）
```
...
rm -f scripts/generate-session-manifest.sh
...
#
# 注意:
#   - `zip -e -P <password>` は password がシェル履歴・プロセスリストに残るため禁止。
#   - 推奨: `zip -e` を引数なしで実行して対話入力する、または `gpg --symmetric` を使う。
#
# 対話入力版（zip）
zip -e remaining-transcripts.zip artifacts/   # ここで password を対話入力する

# あるいは gpg で暗号化（password を対話入力。`--batch --passphrase` は履歴に残るため避ける）
...
| 作業 | 実装形式 | 責務 |
|------|---------|------|
| **Hook / Manifest generation script 作成** | `.claude/hooks/post-commit-manifest` / `scripts/generate-session-manifest.sh` | 決定論的・deterministic ・検証可能 |
...
authoritative_sources:
  ...
  - script_generated:
      ...
      example: "scripts/generate-session-manifest.sh が artifacts/session-manifest.json を出力"
  ...
  - human_attested_from_deterministic_evidence:
      ...
      trustworthiness: high
      evidence: "hook log + human signature / GitHub comment / commit message"
      example: "Hook ログを確認して人間が『この manifest は信頼できる』と comment"
```
...
- [ ] Hook / script / CI のいずれかが deterministic producer として存在する
  ...
  - [ ] または `scripts/generate-session-manifest.sh` がある
  - [ ] または `.github/workflows/session-manifest.yml` がある
- [ ] Schema validation スクリプトが存在
  - [ ] `scripts/validate-manifest.sh` または `jq` が利用可能
...
- 「sessions の自動 push を無効化する flag/設定」 (EntireCLI) または `MANIFEST_SKIP_PUSH=true` (Hook) を **必須**（EntireCLI の実 flag 名は公式 docs を確認）
- Private checkpoint repository を別途作成
- Manual review を毎回実施
- Auto-push を絶対禁止

Public repo での pilot はリスク高。Private repo での先行 pilot を推奨。
