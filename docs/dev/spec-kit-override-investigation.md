...
|---|---|---|---|---|
| `spec-template` | honored by source-inspected | honored by source-inspected | **honored by scratch-e2e** | `.specify/extensions/test-ext/templates/spec-template.md` を配置し `resolve_template spec-template "$PWD"` が当該 path を返すことを確認（§8.4） |
