...
`--paths <target>` による強制 include は **VC baseline fail の証拠とならない**。

- `rg` の `--paths` オプションは検索キーワードの存在に関わらず target を結果に強制 include するため、実装前でも後でも同じ結果を返しうる
- この構造は VC の差分性（baseline fail → 実装後 pass）を破壊する「trivially-pass」パターンである
- `baseline_vc_preflight.py` は `--paths` を使う `rg` / `grep` 系コマンドを `category: trivially_pass` / `decision: blocked` と判定する
- 修正方法: `--paths` を除去し `--keywords`（または検索パターン）のみで目的のパスが返ることを確認する追加 VC を設ける
