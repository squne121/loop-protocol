...
VC は「実装前 baseline で fail し、実装後に pass に変化する」ことを確認するコマンドのみを使う。

- **使用可**: 実装後にのみ存在するシンボル・パターン・ファイルを対象とした `rg` / `grep`（baseline では該当なしで exit 1、実装後に exit 0）
- **使用不可**: 既存コードにすでに存在する文字列・パス・シンボルへの `rg` / `grep`（baseline で exit 0 となり `baseline_fail_expected` にならないため VC として機能しない）
  - この場合は `# preflight-scope: pr_review_only` に振るか、実装後にのみ存在するパターンに変更する

#### trivially-pass 禁止

`match-ssot.sh` / `match_ssot.py` 等の **repository-local discovery script** で `--keywords` と `--paths` を**同時使用**することは、**VC baseline fail の証拠とならない**。

- discovery script の directory mapping は `--paths` 指定パスを keyword 不在でも `matched_documents` に強制 include するため、実装前でも後でも同じパスが返りうる
- この構造は VC の差分性（baseline fail → 実装後 pass）を破壊する「trivially-pass」パターンである
- `baseline_vc_preflight.py` は discovery script に `--keywords` + `--paths` を同時使用するコマンドを `category: trivially_pass` / `decision: blocked` と判定する（呼び出し形態 `bash match-ssot.sh` / 直接実行 / `python3 match_ssot.py` / `uv run python3 match_ssot.py` すべて対象）
- 修正方法: `--paths` を除去し `--keywords` のみで目的のパスが返ることを確認する追加 VC を設ける
- 注意: `rg` / `grep` には `--paths` オプションは**存在しない**。rg はポジショナル PATH 引数と `-g/--glob` でパス指定する。rg/grep のポジショナル path 指定は forced-include ではなく trivially-pass に該当しない

#### `rg` / `grep` exit code の意味

| exit code | 意味 | VC としての扱い |
|---|---|---|
| 0 | 1 件以上マッチした（`unexpected_pass`） | baseline で exit 0 なら VC 設計ミス — 使用不可 |
| 1 | マッチなし（`expected_fail`） | baseline で exit 1 → 実装後 exit 0 が正しい差分 |
| 2 | エラー（構文エラー・ファイル不在等） | VC 実行エラー — block（VC を修正すること） |

### preflight-scope marker の per-command 規約

`# preflight-scope: <scope>` マーカーは **各 `$` コマンド直前 1 行**に記載する。
...
|---|---|---|
| 0 | 全テスト pass | |
| 1 | テスト失敗あり | `expected_baseline_fail` として使用可 |
| 4 | usage error（オプション誤り等） | VC 設計ミス — block |
| 5 | no tests collected | **`expected_baseline_fail` と混同しない**。テスト対象ファイルが存在しない / パスが誤りの可能性 |

exit 5（no tests collected）を「baseline で fail した」と解釈してはならない。テストが収集されていないだけであり、実装後もテストが収集されなければ exit 5 のまま変化しない（偽陽性）。
