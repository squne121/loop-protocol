...
- `iteration >= max_iterations` → `termination_reason: human_escalation`
...
| `issue-reviewer` | Issue レビュー判定 | issue-reviewer | orchestrator | `ISSUE_REVIEW_RESULT_COMPACT_V1` | `status`, `verdict`, `needs_second_pass`, `blocking_issues`, `non_blocking_improvements`, `diff_proposal` | なし | domain judgment の再解釈禁止 | read-only（mutation 禁止） | `review_failed` | `.claude/skills/review-issue/` |
...
| `iteration >= max_iterations` | Step 5 | max_iterations 超過 | `termination_reason: human_escalation` |
