...
- **V2 envelope の組み立て**: orchestrator は child の claim envelope テキストに `PARENT_REPLAY_VERDICT` / `PARENT_REPLAY_ROUTING` / `PARENT_REPLAY_SHOULD_CONSUME` / `PARENT_REPLAY_BODY_SHA256` / `PARENT_REPLAY_NEXT_STATE`（canonical 1 行 JSON）/ `PARENT_REPLAY_BINDING_DIGEST` の 6 行を自ら計算して追記し `ISSUE_REVIEW_RESULT_COMPACT_V2`（15 行）を構成する。routing に使われるのは `PARENT_REPLAY_*` のみであり、V1 の child 自己申告 `REPLAY_VERDICT` 等は producer 契約から廃止された。
