...
### Step 2a: parent-owned replay binding V2 を新規追加する（#1532）

`issue-reviewer` の `needs-fix` 判定に対する arbitration（Step 2a、`reviewer_claim_replay.py`）は、child の isolation worktree が返す raw artifact / raw `REPLAY_*` フィールドを orchestrator が直接信用しない V2 契約へ移行する。

- **child の出力**: `issue-reviewer` は `REVIEWER_BLOCKER_CLAIM_V1`（`ISSUE_REVIEW_RESULT_COMPACT_V1` の needs-fix envelope: STATUS/VERDICT/SUMMARY/BLOCKERS/NEXT_ACTION/MUST_READ/EVIDENCE/ARTIFACT + REPLAY_VERDICT/REPLAY_ROUTING/REPLAY_SHOULD_CONSUME/REPLAY_BODY_SHA256/REPLAY_ARTIFACT_DIGEST）を返す。これは bounded な reviewer claim であり、`REPLAY_NEXT_STATE` の確定値ではない。
- **parent の replay**: orchestrator（parent）は `.claude/skills/issue-refinement-loop/scripts/parent_replay_binding.py` を使い、自ら取得・保存・readback した `review_result` / `readiness_result` / `vc_syntax_result` / `vc_preflight_result` / `previous_state` / identity（`repository_full_name` / `issue_number` / `refinement_session_id` / `iteration_id`）を入力として `reviewer_claim_replay.analyze()` を in-process で再実行し、`PARENT_REPLAY_BINDING_ARTIFACT_V1`（`replay_next_state` と `binding_digest` を含む）を生成する。child の raw artifact ファイルは一切読まない（#1472 isolation boundary を継承）。
- **V2 envelope の組み立て**: orchestrator は child の V1-valid needs-fix envelope テキストに `REPLAY_NEXT_STATE`（`parent_replay_binding.py` の `replay_next_state` を canonical 1 行 JSON 化したもの）と `REPLAY_PARENT_BINDING_DIGEST`（`binding_digest`）の 2 行を自ら追記して `ISSUE_REVIEW_RESULT_COMPACT_V2` を構成する。
- **V2 validator**: `validate_review_compact_output_v2()` は V1 grammar を完全に保持したまま、`REPLAY_NEXT_STATE` / `REPLAY_PARENT_BINDING_DIGEST` を含む 15 行 grammar を検証し、呼び出し元が独立に計算した `expected_replay_next_state` / `expected_parent_binding_digest` と exact 照合する。不一致・不正形式は `human_judgment_required` に fail-closed する。
- **state 永続化**: `reviewer_claim_replay_state_store.py --write-v2` は V2 validator が `validation_status: valid` を返した場合のみ `REPLAY_NEXT_STATE` を永続化する。raw child stdout からの直接書き込みは行わない。
