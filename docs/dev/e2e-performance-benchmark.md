...
`measurements[]` に **named** `test_e2e_ci` と `test_e2e_monolith_responsive` の
両方があり、各 `elapsed_ms` が usable positive numeric duration である場合だけである。
`_layout_aware_provider_baselines()` はこの条件を満たす monolith baseline だけを
self-pair する。両 named phase の absence、片方の absence、zero / negative /
non-numeric duration は partial observation を作らず
`missing_monolith_performance_phase` として evidence error にする。

この reason は Performance fault domain の測定不適格である。したがって provider
post-filter count、`run_details`、receipt の
`performance_eligible_workflow_run_ids` から当該 run を一貫して除外する一方、root
run set から黙って消してはならない。Reliability consumer はこの exact reason だけを
Performance-only allowlist として単独では nonblocking に扱う。unknown reason、identity
binding、Reliability 自身の evidence 欠落を同じ扱いに広げることはない。

`split` の provider critical path は従来どおり同一 `workflow_run_id` の
`max(core_duration, responsive_duration)` である。split で genuine に
`e2e-responsive-matrix` が欠ける場合は既存の
`missing_pair_e2e-responsive-matrix` fail-closed semantics のままであり、monolith
self-pairingや named-phase rule によって緩和されない。
