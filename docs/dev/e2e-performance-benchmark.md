...
- **gate 側（`tests/ci/test_ci_performance_gate.py::_normalize_run_attempt`）は、この collector 側の是正をまだ完全には反映していない。** `run_attempt` 欠落を attempt 1 とみなす後方互換デフォルトを意図的に維持している -- これを collector と同じ厳格なポリシーに統一するには、Issue #2179 の Allowed Paths 外にある satellite テストファイル（`tests/ci/test_ci_performance_gate_paired_critical_path.py` 等、`run_attempt` を一切設定しない固定 fixture を持つ）の修正が必要になり、これは Issue #2179 の Stop Condition に該当するため本 fix_delta ではスコープ外とした（follow-up 候補）。
