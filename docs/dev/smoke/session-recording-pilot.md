...
parent_issue: "#246"
status: in-progress
note_ja: "このファイルは #246 パイロットの実行記録用メタデータ front matter である"
---

# Session Recording Pilot Smoke Test（セッション記録パイロットのスモークテスト記録）
...
## Evidence Storage Boundary（証跡保存の境界）
...
## Required Evidence Links（必要な証跡リンク一覧）
...
### Phase 1: Preflight（プリフライト確認）

Preflight 段階で agent_session_manifest を投稿済みであることを示す記録（内容・手順は変更しない）:
...
### Phase 2: Implementation（実装フェーズ）

実装フェーズで manifest を投稿済みであることを示す記録:
...
### Phase 3: PR Review（PR レビューフェーズ）

PR レビューフェーズで manifest を投稿済みであることを示す記録:
...
### Phase 4: Kill Switch Verification（Kill Switch 検証フェーズ）

Kill Switch の runtime smoke test 結果を記入する（未実施のため未チェック）:
...
### Phase 5: Secret Exposure Scan（Secret 露出スキャンフェーズ）

各種 Secret 露出スキャン結果を記入する（未実施のため未チェック）:
...
### Phase 6: Metrics（メトリクス集計フェーズ）

計測メトリクスを記入する（未計測のため空欄）:
- token_usage: unavailable (ai_self_reported: forbidden)
- latency_ms:
- human_intervention_count:

### Phase 7: Recommendation（最終推奨フェーズ）

パイロットの採否判断（verdict）を記入する（未実施のため未チェック）:
- [ ] verdict posted
