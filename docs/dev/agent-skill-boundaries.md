...
### SubAgent launch ledger writer の Linux 境界（Issue #1503）
...
source を `tmp/subagent-launch-ledger-writer` に build して entry を渡すだけであり、ledger の
read-modify-write、lock、temporary file、replacement を直接実行してはならない。
...
## temp_residue_classification/v1（Issue #1417 の一時残骸分類契約）
