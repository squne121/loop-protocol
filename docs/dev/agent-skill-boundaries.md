...
| SubAgent | 役割カテゴリ | permissionMode | 主な tools | 禁止 tools |
|---|---|---|---|---|
| `codebase-investigator` | 読み取り専用 | `dontAsk` | Bash, Read | Edit, Write, MultiEdit, Grep, Glob |
| `pr-reviewer` | 読み取り専用 | `dontAsk` | Bash, Read, Grep, Glob | Edit, Write, MultiEdit |
| `test-runner` | 読み取り専用 | `dontAsk` | Read, Grep, Glob, Bash | Edit, Write, MultiEdit |
| `review-issue`（standalone SubAgent） | 書き込み | `acceptEdits` | Bash, Read, Grep, Glob, Write | Edit, MultiEdit |
| `issue-reviewer`（loop worker SubAgent） | 読み取り専用 | `dontAsk` | Bash, Read, Grep, Glob | Agent, Edit, Write, MultiEdit, Skill |
| `issue-author` | 書き込み | `acceptEdits` | Bash, Read, Write | Agent, Edit, MultiEdit |
| `implementation-worker` | 書き込み | `acceptEdits` | Read, Grep, Glob, Bash, Edit, Write, MultiEdit | — |
| `post-merge-cleanup-worker` | cleanup | `default` | Bash, Read | Agent, Edit, Write, MultiEdit |

## Codex Dispatch Guardrail / Codex 振り分け防波堤
...
## Manual Codex Spark Agents / 手動 Codex Spark Agents 群の扱い

`spark-skim` / `spark-worker` / `spark-deep` は manual invocation only の Codex custom subagent として扱う。人間が agent 名を明示 spawn した場合だけ使い、workflow root の auto dispatch 先にしない。

### Reasoning routing 方針 / 推論ルーティング方針

| Agent | 権限 profile | 想定用途 | 補足 |
|---|---|---|---|
...
- `spark-skim`: 日本語要約例。`Spawn spark-skim and read only docs/dev/agent-skill-boundaries.md plus .codex/agents. Return SPARK_AGENT_RESULT_V1 with evidence refs only.`
...
### Positive smoke / negative smoke 方針 / 正負 smoke 方針

- positive smoke: 人間が `spark-skim` / `spark-worker` / `spark-deep` をそれぞれ明示 spawn し、`SUBAGENT_LAUNCH_LEDGER_V1.launches[]` に対象 agent 名、runtime model、reasoning effort、default permissions が記録されることを確認する。
...
- `Codex CLI version`（CLI バージョン）
- `install route`（導入経路）
- `auth mode category`（認証モード分類）
- `agent prompt`（agent 起動 prompt）
- `ledger path`（ledger 保存先）
- `Known limitation`（既知制約）
...
## issue-refinement-loop Producer / Publisher 責務分割 / 生成者と公開者の境界（#1154 / #1165 / #1166）
...
### speckit-implement: direct execution prohibited / 直接実行禁止
...
## Follow-up Materialization Policy / 後続 Issue 具体化ポリシー
...
### child materialization executor (`materialize_child_issues.py`) / 子 issue 具体化 executor 実行境界
...
## Loop Sequencing & Preconditions / ループ順序と事前条件
...
## Mandatory SubAgent Contract / 必須 SubAgent 契約
...
> **APPROVE には各 required phase ごとに PR body 以外の証跡が最低 1 件必要です。**
> PR body ledger is a summary, not the authoritative record.

## SubAgent Execution Ledger / SubAgent 実行台帳
...
## Hook-based Ledger Optional Design / hook ベース台帳の任意設計
...
（Issue #1166 — hooks: `publish_termination_report` 用の controlled mutation policy 制御方針）
...
### Anti-split-brain（AC17）/ 二重系不整合の防止
...
### Module realpath 検証（AC16）/ 実体パス検証
...
### settings.json wildcard と hook enforcement / wildcard と hook 強制方針
