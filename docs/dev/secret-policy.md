...
Vercel、Netlify、GitHub id-token (OIDC) 等が該当する。
...
| **漏洩時手順** | 1. 対象サービスで即時 revoke（API key 無効化） 2. 新しい Secret で rotate（再発行・CI 更新） 3. GitHub Actions Secrets から旧 Secret を削除 4. コミット履歴に混入した場合は `git filter-repo` で除去 5. 影響範囲（公開リソースへの不正アクセス等）を調査・報告 |
