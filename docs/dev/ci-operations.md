...
| `e2e` | runtime / browser evidence gate | required + evidence | yes | `playwright-report`, `test-results` | expected head で success かつ artifact 可用性必須 |
...
**注意:** `e2e` は単なる runtime gate ではなく、Playwright report / test-results / visual evidence summary を持つ evidence producer でもある。現在の workflow は `playwright-report` と `test-results` を `!cancelled()` 条件で upload し、環境 fingerprint や artifact URL を summary に書く。
