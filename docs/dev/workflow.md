...
|---|---|---|
| semantic planning | `selected` / `deferred` / `duplicate` / `superseded` | `deferred` は正常な routing であり `blocked` ではない |
...
`UNKNOWN`、鮮度・完全性・contract review・CI・review・security・permission・publication safety は別 namespace の停止または再実行条件として残す。

### Graph、provenance、consumer compatibility

- graph edge は `source_issue_number -> target_issue_number` で、relation は `depends_on` / `supersedes` / `absorbs` / `duplicates` の閉集合である。node/edge は canonical sort し、self-edge、unknown node、contradictory parallel edge、`depends_on` cycle を拒否する。
- identity、collection completeness、unresolved references、producer / policy / schema version、canonicalization id、`decision_inputs_sha256` と `artifact_sha256` を分離して記録する。
- consumer は supported schema version と projection compatibility を検証し、unknown field・必須キー欠落・型/enum 不正・unsafe decision の PR mutation を拒否する。

### Migration と hard gate

移行順序は `dual-write` → `dual-read` → `equivalence` → `new-authoritative` → `old removal` とする。
old removal の前に consumer inventory と、unsafe decision で PR mutation が呼ばれない negative test を確認する。
