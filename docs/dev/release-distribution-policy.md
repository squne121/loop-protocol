...
| 公開範囲 | GitHub Pages は通常 public に公開される。private repo 由来でも public になり得るため Public Hosting Risk Gate を必ず通過する。Enterprise Cloud の private Pages を使う場合のみ access control 設定を別途確認する |
...
| パス長 | ファイル名を含むパス長は **240 文字以下** |
| ファイル名エンコーディング | ファイル名は **UTF-8 encoding** を使用する |
