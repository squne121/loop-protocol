...
| running-hud | screenshot-baseline | legacy-current | `tests/e2e/__screenshots__/m2-combat-mvp.spec.ts/m2-running-hud-baseline.png`（`m2-combat-mvp.spec.ts` の running HUD baseline test） | #681 / #726 / #727 | running HUD が描画されること・HULL/HP の小数露出がないこと・桁溢れがないこと | 色味 / 詳細配置 / right rail 依存は再設計まで可変 | `maxDiffPixels: 1`（理由: legacy-current。#727 再設計時に再評価する） | #727 再開時に破棄 / 再分類可 | #727（HUD/layout 再設計） |
