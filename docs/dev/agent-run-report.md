...
補足: `VC_ADJUDICATION_RESULT_V1` の `stdout` は compact JSON のみを返し、`raw_stdout` / `raw_stderr` / `full_command_output` は private artifact 側に閉じ、public-safe の artifact では `artifact_ref` + `artifact_digest` で参照する。
