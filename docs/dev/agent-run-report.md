...
- **P0-2 fix（PR #2496 OWNER 敵対的レビュー issuecomment-5546874328）**: `chatgpt_retro_context` へ渡す context payload は `postAgentRunReport` / `updateRetroIndex` の **実際の戻り値**（`comment_url` / `sha256` digest / `canonical_index_digest` / `source_comment_set_digest`）から `buildChatgptRetroContextPayloadFromResults()`（`scripts/agent-logs/lib/chatgpt-retro-context-marker-helper.mjs`）が構築する。事前生成済み `--chatgpt-payload-markdown-file` の内容をそのまま3番目のupsertへ渡すことはしない（既存 `CHATGPT_RETRO_CONTEXT_V1` digest-chain resolver との参照整合を壊すため）
- `--chatgpt-payload-markdown-file` は現在 optional であり、指定された場合も `marker_kind` / `safety` / `prerequisites`（すべて固定 pilot-governance metadata。schema 上 const か、`#1157`/`#1220`/`#1221`/`#1222`/`#1223` を参照する固定値）の supplemental template としてのみ使われる。comment URL・digest の authority は常に実際の upsert 結果から得る。ファイルが読み込めない・パース失敗の場合は `buildChatgptRetroContextPayloadFromResults()` の固定 defaults（`CHATGPT_RETRO_CONTEXT_SAFETY_DEFAULTS` / `CHATGPT_RETRO_CONTEXT_PREREQUISITES_DEFAULTS`）にフォールバックする（fresh completion をブロックしない）
- `agent_run_report` または `retro_index` のいずれかがこの invocation で `failed` になった場合、または実コメント参照（`comment_url`）がまだ存在しない場合（例: dry-run の初回 create）は、`chatgpt_retro_context` の upsert 自体を試行せず、`status: failed` / `reason_code: prerequisite_failed` として記録する（既存 `created|updated|unchanged|failed` の語彙内。新しい状態は追加しない）。同一 completion command を再実行し、両方の prerequisite が実参照を持つに至って初めて marker を作成する
