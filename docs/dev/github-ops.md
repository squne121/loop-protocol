...
**解除タイミング**: triage 完了後（`state/queued` / `state/needs-human` / close のいずれかに移行した時点）。

**state ラベルとの関係**: `triage-required` は state ラベルと競合しない補助ラベル。triage 完了前は `state/queued` を付与しない（未評価の Issue を即 queued にしない）。
...
triage セッション（人間または AI エージェント）
        ↓
  有効 → triage-required 除去 + phase/ ラベル + state/queued
