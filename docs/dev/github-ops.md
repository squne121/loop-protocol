...
（子 Issue 完了・測定準備完了・品質検証済み・ルーティング完了・意思決定記録のいずれかを示す列挙値）
...
worktree・checkout・stash 操作を許可する方針の説明。
