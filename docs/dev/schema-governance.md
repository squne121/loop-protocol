...
**live 検証結果（2026-08-04 時点、実 AGY 1.1.9 に対する `--allow-live` 実行）**: `ephemeralMessage` 注入方式は実際に機能する ── `PreInvocation` hook は正しく発火し（`pre_invocation_hook_started: true`、`pre_invocation_context_accepted: true`）、`command`/`read`/`network` の 3 capability は 1 ラウンド目で compliant になった（`allow`（`grounded_research`）・`deny`（`no_tools`）の両 profile で再現）。この時点では `write` capability のみ、3 ラウンドすべてで `PreToolUse` が一度も観測されず `prompt_noncompliance`（`compliant: false`, `attempts: 3`）となり、両 profile とも `EXIT_PROMPT_NONCOMPLIANT`(78) で終了していた（`child_returncode: 0`。捏造した exit 0 ではない）。`write_to_file` の正式な引数キー名（`TargetFile`/`Overwrite`/`CodeContent`）が実際の official schema と一致していない可能性が高いことが疑われていた（77 行目で既述の通り、これらのキー名は独立確認できていなかった）。

**その後（実 AGY 1.1.10 まで）に上記 non-compliance を解消し、genuine `exit 0` に到達した。** live 投機的プローブで `write_to_file` の実 tool call には常に `Description` 引数が含まれることを確認して `attempt_args["write"]` に追加し、`write` capability も両 profile でラウンド 1 から compliant になった（`EXIT_PROMPT_NONCOMPLIANT` は解消）。さらに、実 AGY が deny 期待の attempt でも相関する `PostToolUse` を発火させる実挙動を `characterize_and_record`（新設の additive field `deny_post_tool_use_characterization`）として正しく扱うよう修正し、加えて allow profile でのみ発現する loopback canary の shutdown race（`socketserver.ThreadingMixIn` の既定 `block_on_close=True` による無期限 join）も修正した。これらすべての修正を適用した結果、`command`/`write`/`read`/`network` の 4 capability 全てが両 profile でラウンド 1 から compliant になり、`grounded_research`（AC3, allow）・`no_tools`（AC4, deny）の両 profile が繰り返し（AC3 は独立 3 回、AC4 は独立 2 回、クリーンな artifact-dir）で genuine `exit 0` に到達することを確認した（`cleanup` 全フィールド `true`、`failure_taxonomy.completion == true`）。本 Issue の Delivery Rule（`live runner exit 0` が両 profile で得られること）を満たしたため、本 PR は `Closes #1979` を使用する。
...
**2026-08-05 是正（squne121 の第二ラウンド敵対的レビュー対応、PR #1994）**: 上記の「genuine exit 0 に到達した」という記述は、その時点で未発見だった以下の欠陥を抱えたままの exit 0 判定であったことが判明し、本追記で是正する。

- **P0-1（deny 側 PostToolUse 成否の未判定）**: `_attempts_from_parent_observation()` の deny 側 `post_tool_use_matches_expectation` は correlation と secret-scan のみを見ており、correlate した `PostToolUse` の `error_digest` が `null`（＝実ツール呼び出しが deny にもかかわらず成功した、真の permission-boundary breach）であるケースを見逃していた。correlate した `PostToolUse` の `error_digest` が `null` の場合は deny attempt を FAIL させ、`error_digest` が非 null（ツール呼び出し自体が失敗/拒否された）の場合のみ `characterize_and_record` として許容するよう修正した。
- **P0-2（capability_gate が起動時のまま固定・aggregate validator が未呼出）**: live runner が genuine exit 0（全 capability が prompt compliant、allow/deny 判定成功）に到達した後も、artifact の `capability_gate.status` は起動時の static preflight 結果（通常 `inconclusive`）に固定されたままだった。加えて `validate_aggregate_manifest()` は定義されているだけで runner 自身の実行経路からは一度も呼び出されておらず（test からのみ呼ばれていた）、aggregate 不変条件は実運用で一度も検証されていなかった。修正: (1) genuine exit 0 到達時に `capability_gate` を `preflight_status`（元の static 結果を保持）・`status: "supported"`・`observed_status: "supported"`・`evidence_source: "runtime_semantic_observation"`・`observation_digest`（実際の attempt 証跡へのバインディング）へ昇格するようにした。(2) `_maybe_write_aggregate_manifest()` が `validate_aggregate_manifest()` を実際に呼び出し、その結果を `aggregate/validation.json` として永続化するようにした。
- **P1-2（`loopback_servers_stopped` がハンドラスレッド不在を証明しない）**: `block_on_close=False` / `daemon_threads=True` は listening socket の close と accept-loop スレッドの終了のみを保証し、per-request handler スレッドの終了は保証しない（`socketserver` 自身のドキュメント通り）。`Handler.setup`/`Handler.finish` で稼働中の handler スレッド ID を実際に追跡する集合を追加し、`_LoopbackCanary.stop()` がその集合に対して bounded に（既定 3 秒）実際の空集合到達を待ってから結果を返すよう修正した。
- **P1-3（secret-safe scan が scan 用コピーのみを sanitize）**: `validate_artifact()` の secret scan は artifact の deep copy 上でのみ `runner.binary_identity.realpath` を `None` にしてスキャンしており、実際にディスクへ書き込まれる/呼び出し元へ返される artifact には生の絶対パス（ホストのユーザー名を含む）がそのまま残っていた。sanitize をソース側（`preflight_agy.py::compute_binary_identity()`）へ移し、`realpath`（生の絶対パス）を廃止して `realpath_class`（`$HOME` 相対化またはベース名のみの汎用化表現）と `realpath_digest`（実パスのハッシュ、drift 比較用）に置き換えた。これにより `validate_artifact()` の scan-only carve-out（deep copy して `realpath` を blank にする処理）自体が不要になり撤廃した。

`binary_identity.realpath` → `realpath_class`/`realpath_digest` への置換は additive ではなく **breaking な local-contract 修正**である（`required` フィールド名の変更、`additionalProperties: false` により旧 shape の artifact はこの修正後の schema でスキーマ無効になる）。66 行目に既述の通り、この schema の consumer は本 PR 群の producer/validator/test のみであり外部配布実績がないため、`v2` への bump は行わずこの場で是正する。
