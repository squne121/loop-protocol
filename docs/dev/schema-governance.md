...
`agy_permission_boundary_e2e/v1` は #1814 の additive schema である。producer は `run_agy_permission_boundary_e2e.py`、実装済み consumer は `test_agy_permission_boundary_runner.py` の `validate_artifact()` / Draft 2020-12 validator test、運用 consumer は reviewer である。secret-safe redaction、`fallback.used: false`、binary / artifact digest、attempt predicate を検証する。Compatibility Decision は additive（既存 artifact consumer を変更しない）。artifact digest は `artifact.digest` と `runner.artifact_digest` を null とした canonical UTF-8 JSON（sorted keys、compact separators）の SHA-256 であり、保存ファイル全体の hash ではない。
