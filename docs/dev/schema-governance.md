...
`agy_permission_boundary_e2e/v1` は #1814 の additive schema である。producer は `run_agy_permission_boundary_e2e.py`、production consumer は同 runner の `validate_artifact()`（`Draft202012Validator` と cross-field invariant）、test consumer は `test_agy_permission_boundary_runner.py`、運用 consumer は reviewer である。schema は五 capability の unique matrix と同数の attempt、parent runner が記録する capability ごとの完全な相関／predicate 集合、identity／child return code を固定する。`diagnostic_ledger` は isolated runtime cleanup 前に集約する PreInvocation 起動・context 受理・注入 step 数と enforcement / PreToolUse / PostToolUse の件数だけを保持し、raw payload・args・prompt・absolute path を保持しない。AC5 の Issue 記載どおりの `--profile no_tools --artifact-dir ...` 呼び出しは、`--allow-live` がないため live 実行前に schema-valid な exit 77 artifact を返す cost-guarded preflight であり、AGY の discovery、version probe、subprocess 実行を行わない。追加課金なしを別途確認した caller だけが `--allow-live` を明示でき、その場合でも exit 0 は canonical AGY identity・child return 0・全 predicate・cleanup・`failure.class: none` に束縛する。exit 77 は unavailable／non-completion／`actual_agy_executed: false` に、cleanup failure は exit 1 に束縛する。runner-local の file mode/readback check は security boundary ではなく fail-closed local guardrail である。secret-safe redaction、`fallback.used: false`、binary / artifact digest も検証する。Compatibility Decision は additive（既存 artifact consumer を変更しない）。artifact digest は `artifact.digest` と `runner.artifact_digest` を null とした canonical UTF-8 JSON（sorted keys、compact separators）の SHA-256 であり、保存ファイル全体の hash ではない。
