...
- **publish lane controlled transaction（#1688）**: `codex-hook-adapter.mjs` は hook process と fresh git probe から `CONTROLLED_PUBLISH_CONTEXT_V1`（repository / issue / active branch / HEAD / remote / Allowed Paths digest と既存の head/gate binding）を組み立て、JSON 引数で `git_mutation_command_policy.py` に明示注入する。canonical `rtk git push origin HEAD:refs/heads/<active-branch>` は policy 内の controlled transaction が verify → SHA-bound push 1 回 → 同じ remote ref の readback を完了した場合だけ成功とする。hook は外側の shell push を deny するため、同じ command が二重実行されない。安全判定ロジックは policy に一元化し、adapter は context 作成と結果の表示だけを担当する。
- terminal command の `env LOOP_PUBLISH_...=... rtk git push ...` は injection ではなく untrusted command text として `context_invalid` で拒否する。hook process の trusted-local 環境から作られた invocation-scoped JSON だけが context source であり、環境変数の永続化や command 内展開は行わない。
...
- `CONTROLLED_PUBLISH_CONTEXT_V1` は trusted local development の cooperative guardrail であり、managed host provenance、malicious same-UID process に対する防御、暗号学的 attestation、分散 strict exactly-once を提供しない。temporary bare remote の fixture は push 回数と readback の回帰を検証するが、live host trust の証明ではない。

### existing_branch_update / initial_branch_create レーンの event boundary と timeout budget（#1688 fix delta）

- **event boundary gate（EVENT-BOUNDARY-01 の是正）**: `codex-hook-adapter.mjs` の `classifyRtkGitPushPublishLane(command, cwd, eventName)` は、`eventName === 'PreToolUse'` のときにしか `git_mutation_command_policy.py` を呼び出さない。`PermissionRequest` は「これは許可されるか」を尋ねるだけの予告イベントであり、最終的な承認判断はまだ確定していない（承認/拒否が覆る可能性がある）ため、`PermissionRequest` 自体が実際の remote write を引き起こしてはならない。`--force-with-lease=` 完全限定 empty-expect lease の initial_branch_create レーン候補も、同じ呼び出し口だけを経由するため同じ gate で守られる。
- **second-layer defense（`boundary_layer` の fail-closed 検証）**: `git_mutation_command_policy.py` 側にも独立した第二の防御層を追加した。`execute_existing_branch_update_transaction` / `execute_initial_branch_create_transaction` の実行は、`boundary_layer` が `_AUTHORIZED_EXECUTION_BOUNDARY_LAYERS`（`worktree_scope_guard_denied` — Claude Code 側の PreToolUse 相当の trusted in-process caller である `scripts/agent-guards/worktree_scope_guard.py` の既定値、および `codex_hook_adapter_pretooluse` — Codex `PreToolUse` hook）のいずれかに一致する場合にのみ許可する。それ以外（`PermissionRequest` 起点や、未知の `--boundary-layer` を与えた直接ターミナル呼び出しを含む）は `execution_not_authorized_for_boundary_layer` として deny し、実行そのものに到達しない。これは暗号学的な caller 認証ではなく（`--boundary-layer` 文字列自体は秘密ではない）、あくまで具体的な PermissionRequest-mutates-remote の穴を塞ぐための fail-closed default であり、既存の origin-remote-identity 検証・Allowed Paths digest 検証・head/gate binding 検証と組み合わせて多層防御を構成する（`test_direct_policy_cli_is_blocked_by_real_hook_chain`）。
- **timeout budget（nested_timeout_mismatch の是正）**: `execute_existing_branch_update_transaction` は `_EXISTING_BRANCH_UPDATE_DEADLINE_SECONDS`（既定 20 秒）という単一の絶対 wall-clock 予算を自分自身で所有し、probe → push → readback の全ステップをこの予算内に収める。`codex-hook-adapter.mjs` 側の `EXISTING_BRANCH_PUBLISH_LANE_TIMEOUT_MS`（既定 27,000ms）はこの Python 側の予算より明確に大きく、かつ `.codex/hooks.json` の hook 全体 30 秒予算より明確に小さい値としてマージンを持たせている（Node が Python プロセスを外側から SIGTERM する前に、Python 自身が構造化された結果を返し終える設計）。deadline を使い切った場合は push を試みる前でも必ず 1 回の bounded readback を実行し、`status: indeterminate_timeout` / `reason_code: existing_branch_update_deadline_exceeded`（または readback が一致した場合は `..._deadline_exceeded_but_verified`）という構造化された結果を返す。クラッシュ・ハング・無条件の `denied` への畳み込みはしない。push subprocess は `start_new_session=True` で起動し、外部からこの Python プロセス自体が kill された場合でも push の子プロセスが取り残されにくいようにしている。
- **transport error と自プロセス完了の非混同（transport_error_conflated_with_own_success の是正）**: `existing_branch_update_completed` / `reason_code: existing_branch_update_completed` は push の return code が 0 かつ readback が一致した、すなわち **この Python プロセス自身の push が観測可能な形で成功した** 場合にのみ返す。push 呼び出し自体がタイムアウトしたり transport error を起こした場合、その後の readback が一致しても `status: indeterminate_timeout` / `reason_code: existing_branch_update_transport_error_but_verified` という別の結果を返す。remote ref が一致したことは証明できても、それが自分の push によるものか並行する別の書き込みによるものかは区別できないため、「exactly-once で完了した」とは主張しない（`test_transport_error_matching_readback_does_not_claim_exact_completion`。initial_branch_create レーンの `transport_error_but_created_and_verified` と同じ設計方針）。
- **probe-to-push race window（TOCTOU）の明記**: `execute_existing_branch_update_transaction` は probe（`classify_remote_branch_state`）と push の間に別プロセスが remote branch を更新する race window を **狭めるが排除しない**。strict compare-and-swap（`--force-with-lease` 等の force 系フラグ）は本 Issue のスコープ外であり追加していない（AC4 が force 系拒否を要求するため）。plain（非 force）push は git 自身の fast-forward チェックにより、race で remote が進んでいた場合は push が拒否され、並行する書き込みを上書きしない（`test_concurrent_remote_update_is_not_overwritten`）。sibling レーンである `execute_initial_branch_create_transaction` の docstring に既にある同種の caveat と揃えた。
