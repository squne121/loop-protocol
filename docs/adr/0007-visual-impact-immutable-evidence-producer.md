...
N/A — Decision が DEFER のため、trust boundary（signer identity / consumed evidence / execution identity / producer responsibility / guarantee boundary）の確定は不要。Option 3 採用時の設計要件は `## Considered Options` の Option 3 に記載した論点を再検討の出発点とする。
...
- GitHub Docs: Reusing workflow configurations（reusable workflow SHA pinning, checkout挙動）
- GitHub Docs: Artifact attestations concept / SLSA v1 Build Level 3ガイド（attestation predicate限界、権限要件）
- GitHub CLI: `gh attestation verify` manual（`--signer-workflow` / `--signer-digest`）
