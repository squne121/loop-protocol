...
- 以下はすべて **failure boundary** であり、各呼び出しを `try`/`catch` で保護する: `getItem` / `setItem` / `JSON.parse`（M3 の save/load contract では `removeItem` を使用しない）。
...
- #739 / #854: reward claim 後に progression snapshot を保存する unit / integration regression（実装・マージ済み）。
- #740: Playwright E2E で save-after-reward・localStorage snapshot retention・no auto-load・combat runtime / result phase 非復元を検証済み（`tests/e2e/m3-loop-mvp.spec.ts`）。
- Load Game UI による snapshot 適用フローのE2Eは本spec時点では未作成。
