...
related_tests:
  ...
  - tests/hud-controller.test.ts
  - tests/player-facing-hp-policy.test.ts
---
...
**HP/HULL numeric policy の narrow fix（Issue #788）**: player-facing normal UI は integer-only ポリシーを適用する。`0 < value < 1` は旧来の `"<1"` ではなく `Math.ceil` で `"1"` を返す。debug / exact inspection はこの formatter を使わず raw value または別 formatter を使う。
...
number_display_policy:
  hp:
    ...
    player_facing_integer_only_policy: true  # Issue #788: normal UI shows integer buckets only
    policy:
      - condition: NaN / Infinity / negative
        output: "0"
      - condition: value === 0
        output: "0"
      - condition: 0 < value (all positive values, including sub-1)
        output: "displayValue = Math.ceil(value); compact boundary is evaluated on displayValue"
        rationale: "Integer-only bucket policy (Issue #788): 0 < value < 1 -> ceil -> 1, avoids showing living unit as 0 AND avoids non-integer string in normal UI"
        sub_rules:
...
- **0 < value（すべての正値）**: `Math.ceil()` で整数 `displayValue` に切り上げ、**その `displayValue` で compact 境界を判定する**。これにより `0 < value < 1` は `Math.ceil(value) = 1` となり、player-facing normal UI では整数のみが表示される（Issue #788 integer-only ポリシー）。旧来の `"<1"` 出力は廃止。`"0"` は撃破済みを示すため `value === 0` 専用とする。浮動小数点アーティファクト（例: `7.9999999` は `8`）も同じ ceil で吸収される。
...
|---|---|---|
| 0 to 9999 | ceil(value) as string | "0", "1", "999", "9999" |
...
### 表示セマンティクス / Display semantics (#726, #788)

- 本表示は **exact HP ではなく、戦闘中に人間が即時認識するための display bucket（残存段階表示）** である。すべての正値を `Math.ceil` で整数に変換するため、表示値は実 HP 以上になり得る（意図的）。
- この関数（`formatCombatNumber`）は **damage log / balance calculation / persistence / score 表示には流用しない**。それらは raw な数値モデルを直接使うこと。表示丸めはレンダリング層に閉じる（state / systems から参照しない）。debug / exact inspection はこの formatter を使わず raw value または別 formatter を使う。
- `0 < value < 1` を `"0"`（撃破済み）と表示しないのは、`CombatSystem` が `hp === 0` を defeated 判定とし `Math.max(0, ...)` でクランプするため、`0 < hp < 1` が生存値だからである。Issue #788 では整数 bucket `"1"` を返すよう変更し、`"<1"` という非整数文字列の露出を廃止した。
