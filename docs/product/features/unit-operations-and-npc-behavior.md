...
- **Duration / TTL**: 固定 tick 数または ms として **deterministic に expire** する。具体値は本書で固定せず、`command intent input` / `targeting policy` follow-up Issue で確定するパラメータとする（placeholder 値は埋めない）。
...
- **Next Action**: 本書 accept 後、§14 の 8-stage に沿って implementation Issue 群を起票し本 Issue（#975）に link する。
