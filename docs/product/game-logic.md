...
- **REQ-LOGIC-VICTORY-001:** Sortie terminates on: player HP ≤ 0 (defeat), timer 30s (normal), enemy units destroyed, or outpost destroyed (victory). For M2, timeout authority is `docs/product/features/sortie.md`..
...
1. **30 秒の sortie timer 満了** → 通常戦闘終了（sortie timer ≥ 30s）
2. **敵拠点破壊** → 敵陣営の designated outpost destroyed → 戦闘終了（victory 扱い）
3. **敵機殲滅** → enemy unit count = 0 → 戦闘終了（victory 扱い）

### authority メモ

docs/product/features/sortie.md は本書の sortie timer authority を M2 で refine & override する上位ドキュメントとして扱い、timeout 実装判断は本節よりこちらに従う。
