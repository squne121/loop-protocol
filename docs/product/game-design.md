...
**Normative boundary (fallback draft status):** Until `docs/product/game-thesis.md` is merged with Human Product Review acceptance, this document is a **fallback draft** and must not override the future accepted game-thesis.

After `game-thesis.md` becomes accepted, this document must be reconciled via a **spec delta issue**（`product-spec-lifecycle.md` §Product Spec Delta Flow 準拠）before it is used as the normative source for `game-logic.md`（C254-5）or `mvp-scope.md`（C254-6）.

## Design Pillars

GDD-level の qualitative design constraint（実装定数を含まない体験設計の指針）:

- **Reverse engineering progression fantasy** — The GDD-level progression fantasy is reverse engineering: combat outcomes should be legible as analyzable enemy technology or tactical learning, not only generic currency. 報酬 surface は「敵戦力の分析・解読」として読み取れる体裁を持つ。
- **Localized high-leverage intervention** — The sortie fantasy is localized high-leverage intervention: the player should perceive that their direct action changes a local battle outcome, rather than merely watching allied AI resolve combat. プレイヤーの直接介入が局所戦況を変えた手応えを優先する。
- **Combat readability as first-order constraint** — combat readability is a first-order design constraint: visual telemetry must support quick tactical decisions without relying on high-fidelity art assets. 戦況把握は低忠実度のアセットでも成立する視覚 telemetry に依存する。

これらは下流の `game-logic.md` / `mvp-scope.md` / `playtest-protocol.md` に伝播する qualitative 制約であり、実装定数（時間 / キー / 経済率 / upgrade 数）の確定は依然下流に委譲する。
...
- **REQ-GDD-004 (Event-driven):** When a playtest-log entry classifies a GDD hypothesis as invalidated, the product-spec process shall require a **spec delta issue** before downstream implementation changes（`product-spec-lifecycle.md` §Product Spec Delta Flow 準拠）.
- **REQ-GDD-005 (Unwanted behavior):** If a GDD update requires implementation constants such as **time budgets, input bindings, economy rates, or upgrade counts**, the GDD shall record them only as Open Questions or Downstream Boundaries and shall not define final values.
...
通常プレイ UI / debug surface の境界、phase-aware HUD、surface taxonomy、viewport / DPR / evidence policy は `docs/product/features/ui-information-architecture.md` を正本として参照する。
...
playtest で検証または棄却される GDD-level の仮説。各 PTH は `playtest-protocol.md`（C254-7）が定義する playtest-log を通じて検証され、invalidated 判定が出た場合は REQ-GDD-004 に従い spec delta issue を経由して GDD を補正する。

- **PTH-GDD-001 — Localized Intervention Agency**
  - Claim: The player perceives their direct intervention as the decisive factor in a local battle outcome.
  - Falsification signal: Players attribute success primarily to allied AI strength or passive stat advantage.
  - Downstream owner: `playtest-protocol.md`, `game-logic.md`

- **PTH-GDD-002 — Reverse Engineering Reward**
  - Claim: Post-sortie reward reads as analyzable technology / capability growth, not only generic currency.
  - Falsification signal: Players describe rewards as cosmetic, arbitrary, or "just numbers."
  - Downstream owner: `mvp-scope.md`, `playtest-protocol.md`
