...
### Commit SHA 注入の確認（AC8）

Evidence Panel の YAML を開き、`app_under_test.commit` フィールドを確認する:

```yaml
app_under_test:
  name: loop-protocol
  commit: "abc1234def5678901234567890123456789012345678"  # 40-char hex SHA（注入成功）
```

- **40文字の hex SHA が入っていれば注入成功**。この値を `tested_commit` として PR コメントに記録する。
- `commit: "unknown"` になっている場合は `commit_unknown_reason` を確認し、`tested_commit: "unknown"` + `tested_commit_unknown_reason` を明記する。
...
app_under_test:
  name: loop-protocol
  commit: "abc1234def5678901234567890123456789012345678"  # 40-char hex SHA（注入成功時）
  # commit: "unknown"                                       # 注入失敗時
  # commit_unknown_reason: "..."                            # 注入失敗時のみ出力（成功時は key absent）
browser:
  version: "125.0.0.0"
...
| `app_under_test.commit` | `tested_commit`（トップレベルフィールド） |
| `app_under_test.commit_unknown_reason` | `tested_commit_unknown_reason`（トップレベル、SHA注入成功時は absent） |
