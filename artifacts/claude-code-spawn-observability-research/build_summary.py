...
def main() -> int:
    ...
    add("")
    add(f"- 総 record 数: {len(records)} 件")
    add(f"- 有効 trial 数: {len(valid)} 件")
    add(f"- 無効（excluded）trial 数: {len(records) - len(valid)} 件")
    ...
    add("- 過去の基準点（historical baseline SHA）: 28394e226533cd59cdfc0f55602ac65e389a6600")
    add("")
