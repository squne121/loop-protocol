...
3. integrity 確認済み stored evidence の `source.limit` を正の整数として検証し、その値以外の caller input は使わず、`check_implementation_overlap.py`（`.claude/skills/implement-issue/scripts/check_implementation_overlap.py`。producer は変更せず subprocess として再実行するのみ）へ同一 `--repo <canonical_repository>` / `--issue-number` / `--limit <stored source.limit>` でオンライン再実行する
...
5. fresh evidence の `source.limit` が正の整数かつ stored `source.limit` と一致すること、さらに fresh evidence の `decision_inputs_sha256` と `expected_decision_inputs_sha256` が一致することを確認する（collection 時点からの drift 検出。上記 2 で stored と expected の同一性が既に確認されているため、fresh がここで一致すれば stored・fresh 双方が同一 collection chain に属することが保証される）
...
overlap_readback_waiver:
  ...
  linked_issue: <int>            # `open_pr.py --linked-issue` と一致必須（自己参照 binding）
  candidates:
    - issue_number: <int>
      updated_at: <ISO 8601>     # fresh evidence の該当 candidate の updated_at と一致必須
      reason: <string>           # `readback_incomplete` で始まり、fresh candidate の reasons に含まれる必須
  expires_on: <YYYY-MM-DD>       # 当日を含めて超過していないこと
  approved_by: <string>
```
...
- 残存限界は、候補 Issue 収集の `source.limit` / `completeness` 契約（#1493）、repository ID（`id` / `node_id`）による binding 未実装（`owner/name` の canonical full_name 小文字化形までの binding）、および PR mutation との非原子性（本節末尾の bounded freshness gate の注記を参照）である。
...
| `E_OVERLAP_PREFLIGHT_EVIDENCE_INVALID` | stored evidence の parse 失敗・スキーマ不一致・embedded `evidence_sha256` と `expected_evidence_sha256` の不一致、stored `source.limit` の欠落・非整数・0以下、または stored `repository` の欠落・型不正・非canonical・`canonical_repository` との不一致（Issue #1470） | evidence file が破損していないか、`expected_evidence_sha256` と stored `source.limit` / `repository` が正しいか確認し、必要なら再収集する |
| `E_OVERLAP_PREFLIGHT_DRIFT` | オンライン再実行の fresh `decision_inputs_sha256` が `expected_decision_inputs_sha256` と不一致、fresh `source.limit` の欠落・非整数・0以下・stored 値との不一致、または fresh `repository` の欠落・`canonical_repository` との不一致（Issue #1470） | `implement-issue/SKILL.md` の overlap preflight（Step 2）を再実行し、新しい evidence で再度 `open_pr.py` を呼び出す |
