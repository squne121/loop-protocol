#!/usr/bin/env bash
# verify_run_evaluation_enrichment_live_cli.sh -- SKIP(77)/FAIL(1)/PASS(0)
# wrapper around test_run_evaluation_enrichment_live_cli.py (Issue #2362
# AC3). Same structure as verify_run_retrospective_live_cli.sh (#2301).
#
...
TEST_FILE="$SCRIPT_DIR/test_run_evaluation_enrichment_live_cli.py"

TESTED_HEAD="$(git -C "$REPO_ROOT" rev-parse HEAD)"
echo "verify_run_evaluation_enrichment_live_cli.sh: tested HEAD: $TESTED_HEAD"
...
while [ $# -gt 0 ]; do
  case "$1" in
    --select)
      if [ -z "${2:-}" ] || [ "${2#--}" != "$2" ]; then
        echo "verify_run_evaluation_enrichment_live_cli.sh: --select requires a non-empty value" >&2
        exit 2
      fi
      SELECT="$2"
      shift 2
      ;;
    *)
      echo "verify_run_evaluation_enrichment_live_cli.sh: unknown argument: $1" >&2
      exit 1
      ;;
  esac
done
...
echo "verify_run_evaluation_enrichment_live_cli.sh: resolved claude binary: $CLAUDE_BIN"
CLAUDE_VERSION="$(claude --version 2>&1)"
echo "verify_run_evaluation_enrichment_live_cli.sh: claude --version: $CLAUDE_VERSION"

if ! (cd "$REPO_ROOT" && uv run --locked pytest "${PYTEST_ARGS[@]}"); then
  echo "FAIL: live CLI enrichment verification failed (see pytest output above)"
  exit 1
fi

echo "PASS: live CLI enrichment verification succeeded"
exit 0
