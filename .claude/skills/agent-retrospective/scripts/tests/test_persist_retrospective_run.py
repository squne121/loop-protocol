...
def test_new_format_installation_token_blocks_publication() -> None:
    ...
    containing exactly 2 ``.`` delimiters and well over 520 characters)
...
def test_classic_ghs_token_blocks_publication() -> None:
    """Issue #2727 AC3: classic legacy-format ``ghs_`` tokens (matching
    ``\\bghs_[A-Za-z0-9]{20,}\\b``) must continue to be rejected by the same
    fail-closed publication gate after the new-format matcher is added."""
    transport = FakeIssueCommentTransport()
    candidate = _new_candidate()
    tainted_delta = [
        {
    ...
    pub_req = _publish_request_dict(
        candidate_records=[candidate], delta_results=tainted_delta, request_id="req-2727-ac3-classic-ghs"
    )
    ctx = pr.AuthorizationContext(tty_confirm=lambda _prompt: True, is_tty=lambda: True)
