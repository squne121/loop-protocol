...
## latitude_runtime_evidence/v1（Latitude CLI runtime evidence、Issue #2375）
...
- 収集は `collect_snapshot.collect_latitude_runtime_evidence()`（`LATITUDE_ALLOWED_ARGV` の
  argv-only 起動、`LATITUDE_TIMEOUT_SECONDS`＝10秒、`LATITUDE_MAX_OUTPUT_BYTES`＝64 KiB、
  allowlisted metric は `trace_count`/`span_count`/`duration_ms` の 3 個のみ）。詳細な CLI
  Boundary/Collection Budget は `execution-budget.md` を参照。
