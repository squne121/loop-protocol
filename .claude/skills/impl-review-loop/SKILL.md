...
本 skill は **control-plane**（state tracking + routing）のみを担当する。各 SubAgent の役割:

| SubAgent | 役割 | 使う skill |
|---|---|---|
| `implementation-worker` | 実装 + 必要なら conflict resolve | `implement-issue` |
| `test-runner` | Verification Commands 実行、mergeable 検知 | （直接 gh + 検証コマンド）|
| `pr-reviewer` | LOOP_VERDICT 判定 | `pr-review-judge` |

publish / mutation などの data-plane 操作（push / `gh pr edit` / マージ）はオーケストレーター自身からは実行せず、対応する SubAgent に委譲する。
