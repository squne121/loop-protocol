...
- `contract_snapshot_url`（任意、省略時は preparation ステップで自動取得）: `issue-contract-review` で `status: go` を返したコメントの URL。未提供の場合は preparation ステップが Issue コメントから `CONTRACT_REVIEW_RESULT_V1 status: go` コメントを自動検出する（存在すれば採用、なければ `issue-contract-review` を自動呼び出し）
