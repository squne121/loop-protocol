...
## branch publish の deterministic retry / safety stop

branch publish が hook / approval 境界または remote head drift で止まった場合、`gh pr create` の再試行前に次の read-only preflight を必須とする。

1. `git fetch origin <branch>` 相当の readback で remote-tracking ref を更新する
2. `uv run --locked python3 scripts/agent-ops/git_ref_probe.py --branch <branch> --remote origin --json` で `current_remote_head` を読む
3. `expected_remote_head`、`current_remote_head`、`local_head`、`verified_head`、`declared_publish_head`、`allowed_paths_gate_status` を `PUBLISH_LANE_DECISION_V1` で照合する
...
PUBLISH_LANE_DECISION_V1:
  ...
  publish_failure_reason:
    ...
    reason_code: remote_write_requires_approval | branch_mismatch | stale_remote_head | local_head_mismatch | mixed_head_contamination | unsafe_wrapper_route
...
1. fetch/readback: remote branch head と local worktree HEAD を読み取る。
...
3. allowed publish lane decision: `remote == origin`、`active_branch == target_branch`、`expected_remote_head == current_remote_head`、`local_head == declared_publish_head`、`local_head == verified_head`、`allowed_paths_gate_status == ok` が全て真の場合だけ `allow_retry` とする。
...
PUBLISH_LANE_DECISION_V1:
  ...
  publish_failure_reason:
    ...
    reason_code: remote_write_requires_approval | hook_policy_denied | branch_mismatch | stale_remote_head | local_head_mismatch | mixed_head_contamination | unsafe_wrapper_route
  ...
  allowed_command: null | "rtk git <publish> origin HEAD:refs/heads/<branch>"
  postcondition: "remote branch head == local_head"
