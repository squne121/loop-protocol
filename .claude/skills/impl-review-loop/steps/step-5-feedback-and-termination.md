...
| merge_state_status | merge_ready |
|---|---|
| `CLEAN` | `true` |
| `UNSTABLE` | 人間判断が必要（`human_escalation`） |
| `BEHIND` | `step-5-mergeability-handling.md` の BEHIND 分岐参照 |
| `BLOCKED` | 人間判断が必要（`human_escalation`） |
| `DIRTY` / `CONFLICTING` | CONFLICTING PR Escalation Runbook 発動 |
| `UNKNOWN` | 5 秒待機 × 最大 3 回 retry 後も UNKNOWN なら `human_escalation` |
