...
ISSUE_SCOPE_ROLLUP_RUN_RESULT_V1:
  ...
  plan: <ISSUE_SCOPE_ROLLUP_PLAN_V2 の JSON>
```
...
| result_sha256 mismatch | `result.result_sha256` が `plan` JSON の sha256 と一致すること |

検証合格時のみ `plan` を `LOOP_STATE.scope_rollup_plan` に採用し、`ISSUE_SCOPE_ROLLUP_PLAN_V2` として以降のステップで使用する。
...
marker 採用後、`plan` の内容に基づいて以下の判断を行う:
