...
- `command_group_key`（`vc_runtime_history.compute_command_group_key()`）: argv・repo-relative cwd・command family から算出する history **グルーピング**専用のキー。option 順序や shell 構文は正規化しない。`applied_timeout_ms` 等の実行パラメータを含まない（timeout が変化しても同じ history bucket を参照し続ける）。
