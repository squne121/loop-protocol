...
**VC canonical format**: `## Verification Commands` セクション内の fenced bash block（` ```bash ... ``` `）が canonical VC format である。各コマンドは `$ <command>` 形式で記載し、コマンド直前行に `# ACN` マーカーを付ける。fenced bash block 以外の形式（インライン code span、プレーンテキスト等）は VC として認識されない。
...
|---|---|---|---|
| `unsupported_shell_syntax` | `$(...)` / backtick / `${...}` を含む、または shlex parse 失敗 | blocked | 実行前静的遮断。run_command() を呼ばない |
| `unsafe_command` | `bash`, `sh`, `python -c`, `python3 -c`, `curl`, `wget`, `rm`, `mv`, `git push`, `git commit` 等の危険コマンド | blocked | 実行前静的遮断。run_command() を呼ばない |
| `command_not_allowed` | allowlist 外のコマンド（default blocked）、`gh api`, `pnpm add`, `npm install` 等 | blocked | 実行前静的遮断。run_command() を呼ばない |
| `trivially_pass` | `match-ssot.sh` / `match_ssot.py` 等の **repository-local discovery script** が `--keywords` と `--paths` を**同時使用**している。directory mapping により `--paths` 指定パスが keyword 不在でも `matched_documents` に強制 include される構造 | blocked | 実行前静的遮断。`--keywords` + `--paths` 併用は VC baseline fail の証拠にならない |
...
分類ロジック（classify_static_command + classify_result 関数）:
0. 静的遮断（classify_static_command、run_command より前）:
   a. `$(...)` / backtick / `${...}` → `unsupported_shell_syntax` / blocked
   b. shlex parse 失敗 → `unsupported_shell_syntax` / blocked
   c. denied command（bash, sh, curl 等） → `unsafe_command` / blocked
   d. git/gh/pnpm mutation subcommand → `unsafe_command` / `command_not_allowed` / blocked
   e. allowlist 外コマンド → `command_not_allowed` / blocked
   f. discovery script（`match-ssot.sh` / `match_ssot.py`）の `--keywords` + `--paths` 同時使用 → `trivially_pass` / blocked（denied-command 検出より先に実施 — `bash match-ssot.sh ...` が `bash` の denied-command 判定より前に正確なカテゴリで捕捉される）
...
#### discovery 系スクリプトの `--paths` 強制 include パターン

`match-ssot.sh` / `match_ssot.py` 等の **repository-local discovery script** が `--paths` を `--keywords` と同時使用すると、directory mapping により指定パスが keyword 不在でも `matched_documents` に強制 include される。これは **AC 単独の証拠にならない**。

- `--keywords` + `--paths` の両方を含む discovery script 呼び出しは `baseline_vc_preflight.py` が `category: trivially_pass` / `decision: blocked` と判定する
- 同じ path が `--keywords` のみ（`--paths` なし）のクエリでも返ることを追加 VC として要求する
- `--paths` forced include は VC の差分性（baseline fail → 実装後 pass）を担保しないため、AC の証拠能力がない
- 注意: `rg` / `grep` には `--paths` オプションは**存在しない**。rg はポジショナル PATH 引数と `-g/--glob` でパス指定する。rg/grep のポジショナル path 引数は forced-include ではなく trivially_pass ではない
