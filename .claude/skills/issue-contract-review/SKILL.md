...
| `trivially_pass` | `rg` / `grep` 系コマンドが `--paths <target>` オプションを使い、検索対象 target を強制 include する構造 | blocked | 実行前静的遮断。`--paths` forced include は VC baseline fail の証拠にならない |
...
0. 静的遮断（classify_static_command、run_command より前）:
   ...
   f. `rg`/`grep` 系の `--paths` forced include → `trivially_pass` / blocked（allowlist 通過後に追加チェック）
...
`rg` / `grep` 等の discovery 系スクリプトで `--paths <target>` オプションを使うと、検索キーワードの存在に関わらず target が結果に強制 include される。これは **AC 単独の証拠にならない**。

- `--paths <target>` を使った VC は `baseline_vc_preflight.py` が `category: trivially_pass` / `decision: blocked` と判定する
- 同じ path が `--paths` なし（`--keywords` のみ）のクエリでも返ることを追加 VC として要求する
