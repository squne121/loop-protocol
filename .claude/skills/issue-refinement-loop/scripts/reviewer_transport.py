...
V2_FIELDS = (
    "SCHEMA", "STATUS", "VERDICT", "SUMMARY", "BLOCKERS", "NEXT_ACTION",
    "MUST_READ", "REVIEWED_BODY_SHA256", "ATTEMPT_ID", "ARTIFACT", "ARTIFACT_SHA256",
)
...
def strict_json_loads(raw: bytes | str) -> Any:
    ...
    def pairs(items: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in items:
            ...
            result[key] = value
        return result
    return json.loads(text, object_pairs_hook=pairs, parse_constant=lambda x: (_ for _ in ()).throw(ValueError("non_finite_json")))
...
def _make_manifest(*, issue_number: int, repo: str, invocation_id: str, attempt: int, backend: str, session_id: str | None, retry_intent: str) -> dict[str, Any]:
    return {
...
def retry_matrix(*, backend: str, initial_session_id: str | None, attempt: int, reason_code: str) -> RetryIntent | None:
    ...
    if attempt >= MAX_ATTEMPTS or reason_code not in {"spawn_failure", "timeout", "signal", "empty_output", "nonzero_exit", "malformed_output", "capture_failure", "artifact_validation_failure"}:
        return None
...
def build_compact_v2(*, verdict: str, summary: str, blockers: int, reviewed_body_sha256: str, attempt_id: str, artifact_relative: str, artifact_sha256: str, must_read: str = "") -> bytes:
    if verdict not in {"approve", "needs-fix"} or blockers < 0 or not _SHA.match(reviewed_body_sha256) or not _ATTEMPT.match(attempt_id) or not _SHA.match(artifact_sha256):
    ...
    values = {
        "SCHEMA": SCHEMA_V2, "STATUS": "ok", "VERDICT": verdict, "SUMMARY": summary,
        "BLOCKERS": str(blockers), "NEXT_ACTION": action, "MUST_READ": must_read,
        "REVIEWED_BODY_SHA256": reviewed_body_sha256, "ATTEMPT_ID": attempt_id,
...
def validate_compact_v2(raw: bytes | str, *, issue_number: int | None = None, invocation_id: str | None = None, attempt: int | None = None) -> dict[str, Any]:
    wire = raw if isinstance(raw, bytes) else raw.encode("utf-8")
    result: dict[str, Any] = {"schema": "REVIEW_COMPACT_VALIDATION_RESULT_V2", "validation_status": "invalid", "normalized_payload": None, "violations": [], "input_sha256": sha256_prefixed(wire), "input_byte_count": len(wire)}
    try:
        text = wire.decode("utf-8", "strict")
    except UnicodeDecodeError:
        result["violations"].append({"code": "utf8_decode_error"}); return result
    if not text.endswith("\n") or "\r" in text or len(wire) > 2048:
        result["violations"].append({"code": "wire_format_invalid"}); return result
    lines = text[:-1].split("\n")
    if len(lines) != len(V2_FIELDS):
        result["violations"].append({"code": "line_count_invalid"}); return result
    values: dict[str, str] = {}
    for expected, line in zip(V2_FIELDS, lines):
        if not line.startswith(expected + ": "):
            result["violations"].append({"code": "field_order_or_name_invalid", "field": expected}); return result
        value = line[len(expected) + 2:]
        if "\x00" in value or "\n" in value:
            result["violations"].append({"code": "control_character"}); return result
        values[expected] = value
    artifact_prefix = "compact_review_result_v2="
    if values["SCHEMA"] != SCHEMA_V2 or values["STATUS"] != "ok" or values["VERDICT"] not in {"approve", "needs-fix"} or values["NEXT_ACTION"] != ("proceed" if values["VERDICT"] == "approve" else "request_changes"):
        result["violations"].append({"code": "value_invalid"}); return result
    if not values["BLOCKERS"].isdigit() or (values["VERDICT"] == "approve") != (values["BLOCKERS"] == "0") or not _SHA.match(values["REVIEWED_BODY_SHA256"]) or not _SHA.match(values["ARTIFACT_SHA256"]) or not _ATTEMPT.match(values["ATTEMPT_ID"]) or not values["ARTIFACT"].startswith(artifact_prefix):
        result["violations"].append({"code": "cross_field_invalid"}); return result
    ...
    try:
        ...
        result["violations"].append({"code": str(exc)}); return result
...
def secure_read_json(*, artifact_root: Path, artifact_relative: str, max_bytes: int = ARTIFACT_MAX_BYTES) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            os.close(leaf_fd); os.close(root_fd)
...
def verify_artifact(*, artifact_root: Path, artifact_relative: str, expected_repo: str, expected_issue: int, expected_body_sha256: str, expected_invocation_id: str, expected_attempt: int, expected_sha256: str) -> dict[str, Any]:
    ...
    expected = {"repository": expected_repo, "issue_number": expected_issue, "reviewed_body_sha256": expected_body_sha256, "invocation_id": expected_invocation_id, "attempt": expected_attempt}
...
def write_semantic_artifact(*, artifact_root: Path, issue_number: int, repo: str, invocation_id: str, attempt: int, reviewed_body_sha256: str, semantic_result: dict[str, Any]) -> tuple[str, str]:
    relative = artifact_relative_path(issue_number, invocation_id, attempt)
    payload = {"schema": ARTIFACT_SCHEMA, "repository": repo, "issue_number": issue_number, "reviewed_body_sha256": reviewed_body_sha256, "invocation_id": invocation_id, "attempt": attempt, "semantic_result": semantic_result}
...
def run_reviewer_transport(*, command: list[str], command_id: str, argv_template_id: str, backend: str, issue_number: int, repo: str, reviewed_body_sha256: str, artifact_root: Path, invocation_id: str | None = None, session_id: str | None = None, per_attempt_deadline: int = PER_ATTEMPT_DEADLINE_SECONDS, total_deadline: int = TOTAL_DEADLINE_SECONDS) -> dict[str, Any]:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        _write_json_once(artifact_root, manifest_relative, _make_manifest(issue_number=issue_number, repo=repo, invocation_id=invocation_id, attempt=attempt, backend=backend, session_id=active_session, retry_intent=intent))
        common = {"invocation_id": invocation_id, "attempt": attempt, "backend": backend, "session_id": active_session, "command_id": command_id, "argv_template_id": argv_template_id, "rendered_argv_sha256": sha256_prefixed("\0".join(command).encode()), "pid": None, "process_group": None, "exit_code": None, "signal": None, "timeout": False, "descendants_reaped": False, "stdout_length": 0, "stdout_sha256": None, "stderr_length": 0, "stderr_sha256": None, "stdout_prefix": "<redacted:0-bytes>", "stdout_suffix": "<redacted:0-bytes>", "stderr_prefix": "<redacted:0-bytes>", "stderr_suffix": "<redacted:0-bytes>"}
        try:
            ...
            result = _attempt_result(**common, failure_phase="spawn", reason_code="spawn_failure", spawn_error=type(exc).__name__)
            ...
            retry = retry_matrix(backend=backend, initial_session_id=session_id, attempt=attempt, reason_code=result["reason_code"])
            if retry is None:
                break
        ...
        stdout: dict[str, Any] = {}; stderr: dict[str, Any] = {}
        threads = [threading.Thread(target=_bounded_reader, args=(process.stdout, STDOUT_CAP, stdout)), threading.Thread(target=_bounded_reader, args=(process.stderr, STDERR_CAP, stderr))]
        ...
        common.update(timeout=timed_out, descendants_reaped=timed_out, exit_code=process.returncode, signal=(-process.returncode if process.returncode and process.returncode < 0 else None), stdout_length=stdout.get("length", 0), stdout_sha256=stdout.get("sha256"), stderr_length=stderr.get("length", 0), stderr_sha256=stderr.get("sha256"), stdout_prefix=_redact_bytes(stdout.get("prefix", b"")), stdout_suffix=_redact_bytes(stdout.get("suffix", b"")), stderr_prefix=_redact_bytes(stderr.get("prefix", b"")), stderr_suffix=_redact_bytes(stderr.get("suffix", b"")))
        if timed_out:
            result = _attempt_result(**common, failure_phase="execution", reason_code="timeout")
        elif process.returncode != 0:
            result = _attempt_result(**common, failure_phase="execution", reason_code=("signal" if common["signal"] else "nonzero_exit"))
            ...
            try:
                ...
                relative, digest = write_semantic_artifact(artifact_root=artifact_root, issue_number=issue_number, repo=repo, invocation_id=invocation_id, attempt=attempt, reviewed_body_sha256=reviewed_body_sha256, semantic_result=semantic)
                wire = build_compact_v2(verdict=verdict, summary=("contract ready" if verdict == "approve" else f"{count} blocker(s)"), blockers=count, reviewed_body_sha256=reviewed_body_sha256, attempt_id=invocation_id, artifact_relative=relative, artifact_sha256=digest)
                compact = validate_compact_v2(wire, issue_number=issue_number, invocation_id=invocation_id, attempt=attempt)
                if compact["validation_status"] != "valid":
                    raise ValueError("parent_compact_validation_failure")
                fields = compact["normalized_payload"]
                verified = verify_artifact(artifact_root=artifact_root, artifact_relative=relative, expected_repo=repo, expected_issue=issue_number, expected_body_sha256=reviewed_body_sha256, expected_invocation_id=invocation_id, expected_attempt=attempt, expected_sha256=digest)
                if verified["status"] != "valid":
                    result = _attempt_result(**common, failure_phase="artifact_validation", reason_code="integrity_failure", artifact_validation=verified)
                else:
                    result = {"schema": ATTEMPT_SCHEMA, "transport_status": "ok", "semantic_verdict": verdict, **common, "failure_phase": None, "reason_code": None, "compact": fields}
            except (ValueError, OSError, json.JSONDecodeError) as exc:
                result = _attempt_result(**common, failure_phase="compact_validation", reason_code="malformed_output", compact_error=type(exc).__name__)
        result_relative = attempt_relative_dir(issue_number, invocation_id, attempt) + "/attempt_result.json"
        _write_json_once(artifact_root, result_relative, result)
        results.append(result)
        if result["transport_status"] == "ok":
            return {"schema": "REVIEWER_TRANSPORT_RESULT_V1", "transport_status": "ok", "semantic_verdict": result["semantic_verdict"], "invocation_id": invocation_id, "attempts": results}
        retry = retry_matrix(backend=backend, initial_session_id=session_id, attempt=attempt, reason_code=result["reason_code"])
        if retry is None: break
        active_session = retry.session_id if retry.session_id is not None else generate_invocation_id()
        retry_intent_kind = retry.kind
    return {"schema": "REVIEWER_TRANSPORT_RESULT_V1", "transport_status": "environment_failure", "semantic_verdict": None, "invocation_id": invocation_id, "attempts": results}
...
def main(argv: list[str] | None = None) -> int:
    ...
    value = validate_compact_v2(raw, issue_number=args.issue_number, invocation_id=args.invocation_id, attempt=args.attempt)
