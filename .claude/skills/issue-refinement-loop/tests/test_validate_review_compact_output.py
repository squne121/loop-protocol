...
GIVEN/WHEN/THEN style tests covering AC1-AC9, AC12, AC13, AC15-AC21 of Issue #1507:
  ...
  - AC6: value constraints (VERDICT/NEXT_ACTION/BLOCKERS)
  ...
  - AC12: parity with real compact_review_result.py output
...
removed, so the needs-fix envelope no longer carries REPLAY_*/
PARENT_REPLAY_*/REVIEWER_BLOCKER_CLAIM fields -- it now has the exact same
8-field shape as the approve envelope (distinguished by VERDICT/NEXT_ACTION/
BLOCKERS values only). The V2 (parent-local replay binding) tests that used
to live at the end of this file were removed along with
`validate_review_compact_output_v2()` / `parent_replay_binding.py`.
"""
...
# ---------------------------------------------------------------------------
# AC2: exact 8-line needs-fix envelope (#1873: same shape as approve)
# ---------------------------------------------------------------------------


class TestNeedsFixEnvelope:
    def test_needs_fix_exact_8_lines_valid(self):
        """GIVEN exact 8-line needs-fix envelope WHEN validated THEN valid."""
        text = _needs_fix_envelope()
        result = validate_review_compact_output(text)
        assert result["validation_status"] == "valid"
        assert result["envelope_kind"] == "needs_fix"
        assert result["violations"] == []
        assert result["normalized_payload"]["VERDICT"] == "needs-fix"

    def test_needs_fix_with_replay_field_present_is_rejected(self):
        """#1873: a stray REPLAY_* field on a needs-fix envelope is an
        unknown field and is rejected -- no production path emits it."""
        text = _needs_fix_envelope() + "\nREPLAY_VERDICT: deterministic_fail_confirmed"
        result = validate_review_compact_output(text)
        assert result["validation_status"] == "invalid"

    def test_needs_fix_missing_artifact_field_is_invalid(self):
        """GIVEN needs-fix envelope missing the last field (ARTIFACT) THEN invalid."""
        text = _needs_fix_envelope()
        truncated = "\n".join(text.split("\n")[:-1])
...
class TestValueConstraints:
    ...
    def test_value_constraints_needs_fix_wrong_next_action_rejected(self):
        """GIVEN a needs-fix envelope with an invalid NEXT_ACTION WHEN
        validated THEN next_action_value_invalid (#1873: no REPLAY_* matrix
        remains -- next_action is the only routing-adjacent value left)."""
        text = _needs_fix_envelope(next_action="proceed")
        result = validate_review_compact_output(text)
        assert result["validation_status"] == "invalid"
        codes = {v["code"] for v in result["violations"]}
        assert "next_action_value_invalid" in codes
...
class TestParityWithRealProducers:
    def test_parity_with_real_producer_output(self, tmp_path, monkeypatch):
        """GIVEN real compact_review_result.py stdout for approve/needs-fix
        fixtures WHEN validated THEN valid (#1873: Replay Arbitration was
        removed -- the needs-fix envelope is now the plain 8-field
        ISSUE_REVIEW_RESULT_COMPACT_V1 shape, no PARENT_REPLAY_*/
        REVIEWER_BLOCKER_CLAIM assembly step).
        ...
        # -- needs-fix parity: real producer, plain 8-field envelope --
        ...
        assert needs_fix_result["normalized_payload"]["VERDICT"] == "needs-fix"
        # #1873: no REPLAY_*/PARENT_REPLAY_*/REVIEWER_BLOCKER_CLAIM fields.
        assert "REVIEWER_BLOCKER_CLAIM" not in needs_fix_result["normalized_payload"]
