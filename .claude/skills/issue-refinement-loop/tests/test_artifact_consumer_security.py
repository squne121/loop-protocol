...
def _artifact(root: Path) -> tuple[str, str]:
    relative, digest = transport.write_semantic_artifact(
        artifact_root=root, issue_number=2054, repo="squne121/loop-protocol",
        invocation_id="secure-invocation", attempt=1,
...
def test_given_bound_regular_file_when_verified_then_same_raw_bytes_are_accepted(tmp_path: Path):
    ...
    result = transport.verify_artifact(
        ...
        expected_issue=2054, expected_body_sha256="sha256:" + "c" * 64,
        expected_invocation_id="secure-invocation", expected_attempt=1, expected_sha256=digest,
    )
...
def test_given_symlink_component_when_verified_then_integrity_failure(tmp_path: Path):
    ...
    result = transport.verify_artifact(
        artifact_root=tmp_path, artifact_relative="evil/secure-invocation/attempt-001/compact_review_result_v2.json",
        expected_repo="squne121/loop-protocol", expected_issue=2054,
        expected_body_sha256="sha256:" + "c" * 64, expected_invocation_id="secure-invocation",
        expected_attempt=1, expected_sha256=digest,
    )
...
def test_given_missing_tampered_or_wrong_bound_artifact_when_verified_then_integrity_failure(tmp_path: Path):
    ...
    assert transport.verify_artifact(**{**expected, "artifact_relative": "2054/secure-invocation/attempt-002/missing.json"})["status"] == "integrity_failure"
    assert transport.verify_artifact(**{**expected, "expected_repo": "other/repository"})["status"] == "integrity_failure"
