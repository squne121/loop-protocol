...
def _snapshot() -> dict:
    return {
        ...
        "issue": {"number": 1037, "body": "test", "title": "test", "labels": []},
        "comments": [],
    }
...
def test_validate_schema_never_raises_for_reference_resolution_failures():
    broken_schema = {
        ...
        "$id": "urn:loop-protocol:broken-validator-test:v1",
        "$ref": "urn:loop-protocol:missing-resource:v1",
    }
...
def test_planner_exit_3_emits_schema_valid_fallback_result(tmp_path, capsys):
    ...
    with (
        ...
        mock.patch.object(wrapper, "_invoke_planner", return_value=(None, 3, "internal", "")),
    ):
        result, exit_code = wrapper.run_preflight(
            issue_number=1037, repo="squne121/loop-protocol", anchor_comment_urls=[], fixture_path=fixture_path
        )
...
def test_failure_artifact_is_readback_schema_valid_before_reporting(tmp_path):
    result, _ = wrapper._emit_failure_result(
        repo_root=tmp_path,
        issue_number=1037,
        repo="squne121/loop-protocol",
        status="environment_failure",
        next_action="fix_environment",
        blockers=[wrapper.BLOCKER_PLANNER_INTERNAL_ERROR],
        planner_exit_code=3,
        planner_fail_closed=True,
        planner_fail_closed_reason_codes=[wrapper.BLOCKER_PLANNER_INTERNAL_ERROR],
        required_sections=[],
        required_contract_keys=[],
        rewrite_constraints=wrapper._build_safe_rewrite_constraints([], []),
