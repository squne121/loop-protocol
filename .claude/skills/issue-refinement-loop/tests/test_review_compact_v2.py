...
def test_given_valid_v2_when_validated_then_exact_eleven_lines_are_accepted():
    raw = transport.build_compact_v2(
        ...
        reviewed_body_sha256="sha256:" + "a" * 64, attempt_id="inv-1",
...
def test_given_missing_out_of_order_or_mismatched_identity_when_validated_then_fail_closed():
    ...
    assert transport.validate_compact_v2(raw.replace(b"2054/inv-1", b"2055/inv-1"), issue_number=2054)["validation_status"] == "invalid"
    assert transport.validate_compact_v2(raw.replace(b"attempt-001", b"attempt-002"), attempt=1)["validation_status"] == "invalid"
...
def test_given_parent_transport_failure_when_router_consumes_receipt_then_environment_failure(tmp_path: Path):
    ...
    run = subprocess.run(
        [sys.executable, str(router), "--loop-state-file", str(loop_state),
         "--reviewer-transport-result-file", str(receipt)],
