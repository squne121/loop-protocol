...
2. **構造的衝突（同一 sub-file anchor への競合変更）**
   ...
   - `scope_context.escalation_required == true`（`conflict_type: same_anchor_conflicting_operation`）または `scope_context.conflict_type == uncertain` のとき `suggested_action: human_review_required` を設定し、人間が判断する。
...
- `scope_context.conflict_type == uncertain`（完全同一ファイルだが機械可読 anchor で disjoint を証明できない場合の fail-safe）
...
orchestrator_rules:
  confidence_high:
    ...
    escalation_override: "scope_context.escalation_required==true または conflict_type==uncertain の候補は suggested_action を human_review_required に上書きする（#550: 語彙一致ではなく構造的衝突で判定）"
  ...
  structural_conflict_candidates:
    action: "scope_context.escalation_required==true / conflict_type==uncertain は human_review_required を設定して即時停止。AI による自動統合禁止"
