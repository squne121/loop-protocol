...
|---|---|
| `human_review_directive` | evidence の `source_kind` が `issue_comment` / `pull_request_review` かつ `author_association` が `OWNER`/`MEMBER`/`COLLABORATOR` |
| `ai_inferred` | evidence が存在しない、`source_kind: generated_by_agent`、または `author_association` が信頼できない（fail-closed、AC13） |
...
2. evidence 欠落 / untrusted author → `ai_inferred` として `human_escalation`（`reason_code: ai_inferred_scope_delta` または `untrusted_author_association`）
...
6. `directive.confidence: explicit` → `contract_update_required`（`reason_code: explicit_human_contract_directive`, `next_step: rerun_refinement_after_contract_update`）
...
contract_patch_plan_v1:
  ...
  base_issue_body_sha256: <sha256 | null>
  source_evidence:
    - source_ref: <string | null>
...
`scope_signal_guard_decision_v2.route == contract_update_required` を受け取ると、
`NEXT_ACTION: proceed_with_contract_update` を返す（`termination_reason` は一切変更しない。loop は継続したまま、
契約更新 → refinement 再実行の non-destructive branch に入る）。sidecar が未指定、またはパース失敗時は
soft-fail（絶対値として扱わず、既存の `scope_signal_guard.triggered` ベースの hard stop 判定にフォールバックする）。
