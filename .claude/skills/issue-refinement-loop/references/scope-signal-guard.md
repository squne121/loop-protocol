...
| `review` | `hard_stop_candidate` | **true** | signal 確認 → `human_escalation` |
...
1. preflight phase では `decide_next_loop_action.py` を呼ばない（`forbidden_routers` に含まれる）
...
4. `decide_next_loop_action.py` は review 以降の phase（`hard_stop_eligible: true`）でのみ呼ぶ
...
- preflight phase で `decide_next_loop_action.py` を呼ぶ（phase gate が forbidden とする）
