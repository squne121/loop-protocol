...
- `canonical_step2_route: step_5_human_judgment_required`（`status: ok` + `compact_result.next_action: human_judgment_required`（`verdict` の値は問わない。既存の compact schema — `review-issue/SKILL.md` の「Producer I/O ownership」節 — で `verdict: needs-fix` かつ `next_action: human_judgment_required` の組が有効であるため）、または `status: input_or_runtime_error` + 既知の `error_code`（`reviewer_transport_environment_failure` / `artifact_readback_failed`）に対応）→ Step 5 (human_review_required) へ
- `canonical_step2_route: fail_closed_environment_or_integrity_failure`（上記いずれにも一致しない組。`status != ok` かつ未知/欠落の `error_code` を含む）→ environment/integrity failure として fail-closed で停止する（`decide_next_loop_action.py` を呼ばず、通常の rewrite loop へも進めない）
