...
**`STATUS: needs_fix` / `NEXT_ACTION: apply_deterministic_structural_repair`（Issue #2402）:**
`structural_repair_action.disposition_summary: auto_apply_safe` を伴うこの canonical next action は、fresh preflight artifact に限り canonical main/root thread が `skill_runtime_exec.py` を通じて `structural_repair_action.apply` へ dispatch する。実行形は `uv run python3 scripts/agent-guards/skill_runtime_exec.py --command-id structural_repair_action.apply --issue-number <N> --repo <owner/repo> --apply-structural-repair-action <preflight_result_path>` である。`<preflight_result_path>` は preflight の absolute `ARTIFACT:` 表示をそのまま渡さず、canonical main root からの repo-relative `.claude/artifacts/issue-refinement-loop/<N>/refinement_preflight_result_v1.json` に正規化する。対象 Issue 自身の artifact root、canonical root/default branch、及び `LOOP_ISSUE_NUMBER=<N>` に対応する active Issue worktree が必須である。

この lane は generic `apply_deterministic_repair → repair_action.apply → --apply-repair-action` と独立している。structural command に generic flag、generic command に structural flag、両 flag の混在、追加 argv、他 Issue・absolute・traversal・symlink artifact path は dispatch 前に reject する。root は command ID / outer flag / repo / Issue / path / cwd / branch / active-worktree context だけを検証し、nested structural payload、disposition、digest、provenance、body drift を再解析しない。これらは既存 structural consumer の責務である。

`structural_repair_action.apply` の後に fresh preflight と fresh Step 2 review を再開できるのは、**exit 0** かつ **`phase: complete`** かつ **`failure_code: null`** かつ **`mutation_outcome` が `applied` または `no_change`** の場合だけである。それ以外（`unknown`、`not_attempted`、incomplete phase、nonzero exit を含む）は implementation authorization を出さず停止する。`unknown` / `not_attempted` / nonzero result を盲目的に再試行しない。
...
にそのまま従う（承認なら Step 2.5、要修正なら Step 4、human_judgment_required なら Step 5、
不整合なら fail-closed で停止）。
