...
- `max_iterations`（任意、デフォルト 1）: 上限イテレーション数
...
[Step 2: レビュー] → issue-reviewer SubAgent
   ...
   ─ if iteration + 1 >= max_iterations:
        Step 4 を実行せず termination_reason: needs_second_pass で停止
   ─ else:
        ↓
[Step 4: 本文改善] → issue-author SubAgent (edit-issue skill 経由)
        ↓
   iteration += 1 → Step 1 へ
```

> **iteration semantics**: `iteration` は **完了した review cycle**（Step 1 → Step 2）の本数を表す。`max_iterations` は許容 review cycle 数の上限。`max_iterations=1` 既定では、Step 2 の verdict が `needs-fix` でも Step 4 を実行せず即 `needs_second_pass` で停止する（次の review cycle を回さない）。
...
LOOP_STATE:
  ...
  investigation_policy:
    codebase_required: false           # true で Step 1 実行
    codebase_reason: null
    target_paths: []
    repo_claims: []
    skip_reason: null
  web_research:             # #203 retry 拡張点用に継続
    required: false
    status: null             # ok | failed | skipped
    failure_class: null      # #203 retry で使用
    verification_route: null
    result: null             # WEB_RESEARCH_RESULT_V1 または null
    failure_reason: null
    critical_claims: []
  web_research_policy:
    required: false          # true で Step 1b 実行
    reason: null
...
WEB_RESEARCH_RESULT_V1 handling:
  # critical 判定の SoT: LOOP_STATE.web_research_policy.critical_external_claims（Step 0f sync_rule で web_research.critical_claims にも複製済み）
  critical_claims_source: LOOP_STATE.web_research_policy.critical_external_claims

  # 共通副作用: LOOP_STATE.web_research.{status, failure_class, verification_route, result} を WEB_RESEARCH_RESULT_V1 から更新する
  always:
    update LOOP_STATE.web_research.{status, failure_class, verification_route, result}
  ...
  if status == failed:
    if web_research_policy.critical_external_claims is non-empty:
      termination_reason: human_escalation
      reason: web_research_failed_critical  # query_error / fallback 失敗を含む
    else:
      record failure_class in LOOP_STATE.web_research
      proceed_to: Step 2
```
...
|---|---|
| Step 2: `verdict=approve` | `approved` |
| Step 2: `needs-fix` かつ `iteration + 1 >= max_iterations`（Step 4 実行せず停止） | `needs_second_pass` |
| 各 Step: `human_review_required=true` | `human_escalation` |
| `final_classification=superseded_by_decision` + Step 0c 完了 | `superseded_by_decision` |

### NEEDS_SECOND_PASS 品質エスケープハッチ
`iteration` は完了 review cycle 数。Step 2 が `needs-fix` を返したとき、`iteration + 1 >= max_iterations` であれば **Step 4 を実行せず** `termination_reason: needs_second_pass` で停止する（自動再ループ禁止）。`max_iterations=1` 既定では初回 `needs-fix` で必ず本ハッチが発火するため、blocking ambiguity を黙って 1 周で打ち切らない。`max_iterations > 1` の明示指定または human decision で次パスを許可する。
## Guardrails
