...
orchestrator は child の claim envelope に、上記 artifact から計算した `PARENT_REPLAY_VERDICT` / `PARENT_REPLAY_ROUTING` / `PARENT_REPLAY_SHOULD_CONSUME` / `PARENT_REPLAY_BODY_SHA256` / `PARENT_REPLAY_NEXT_STATE`（canonical 1 行 JSON）/ `PARENT_REPLAY_BINDING_DIGEST` の 6 行を追記して `ISSUE_REVIEW_RESULT_COMPACT_V2`（15 行）を構成し、`validate_review_compact_output.py --v2`（`review_compact.validate_v2`、`--binding-artifact-file` 等すべて必須、High-1）で binding artifact の strict schema・digest 再計算・identity/body 照合・envelope 全フィールドの exact 照合を経てから consume する。`PARENT_REPLAY_VERDICT` は `reviewer_claim_replay.py` の `_LEGACY_VERDICT_MAP_V1` と同期した5値 enum。
