...
- `canonical_step2_route: step_4`（`status: ok` + `compact_result.verdict: needs-fix` + `compact_result.next_action: request_changes` に対応。`failure_class` が欠落・`None` の場合はこちら）→ `decide_next_loop_action.py`（rewrite 後 / next-action 決定時に呼ぶ、下記「LOOP_STATE」参照）へ VERDICT をそのまま渡す（**#1873: 旧 Step 2a Replay Arbitration は撤去された** — orchestrator は VERDICT を独立に再計算せず直接信頼する）。同一 triple かつ同一 payload の verified `merged_review_result.failure_class == "contract_readiness_human_judgment"`（Issue #2397。`contract_readiness_human_judgment` は環境・tool・timeout・unknown 分類など Issue 本文書き換えでは解消しない operator 起因の readiness 状態で、genuine semantic / owner ambiguity 用の `step_5_human_judgment_required` とは分離する。compact V2 wire の `NEXT_ACTION: proceed | request_changes` 二値契約は変更しない）の場合のみ、代わりに `canonical_step2_route: step_5_operator_intervention_required` を返し、既存の Step 5 handler を再利用して `termination_cause: operator_intervention_required` として区別して停止する（詳細は `issue-refinement-loop/references/termination-policy.md` 参照）
