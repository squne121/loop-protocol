...
validator（`validate_review_compact_output.py`）呼び出し順序の SSOT は Step 2 冒頭を参照し、ここでは重複記載しない。以下は validator 通過後に `normalized_payload` 経由で consume する REPLAY フィールドの形式:
...
**Parent-owned replay binding V2（Issue #1532）**: 上記 `REPLAY_NEXT_STATE` を child stdout からそのまま `--write` する経路は V1 の残余リスクである（child self-claim を provenance と誤認しうる）。V2 では orchestrator が `parent_replay_binding.py` を使い、自ら取得・保存・readback した `review_result` / `readiness_result` / `vc_syntax_result` / `vc_preflight_result` / `previous_state` / identity（`repository_full_name` / `issue_number` / `refinement_session_id` / `iteration_id`）を入力として `reviewer_claim_replay.analyze()` を parent 側で独立に再実行し、`PARENT_REPLAY_BINDING_ARTIFACT_V1`（`replay_next_state` と `binding_digest`）を生成する。orchestrator は child の V1-valid needs-fix envelope に `REPLAY_NEXT_STATE`（canonical 1 行 JSON）と `REPLAY_PARENT_BINDING_DIGEST`（`binding_digest`）を自ら追記して `ISSUE_REVIEW_RESULT_COMPACT_V2` を構成し、`validate_review_compact_output_v2()` で `expected_replay_next_state` / `expected_parent_binding_digest` と exact 照合してから `reviewer_claim_replay_state_store.py --write-v2` へ渡す。`--write-v2` は `validation_status: valid` の場合のみ `REPLAY_NEXT_STATE` を永続化する（raw child stdout の直接 `--write` は V2 経路では行わない）。詳細は `docs/dev/workflows/issue-refinement-loop-design.md` の「Step 2a: parent-owned replay binding V2」を参照する。
