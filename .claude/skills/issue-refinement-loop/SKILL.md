...
# iter 3: 旧経路（anchor comment snapshot を Step 4 に直注入）がないこと
# Step 4 の手順本文に raw snapshot の直接渡しが残っていないことを人的確認:
# - "Step 4 は raw anchor comment（LOOP_STATE.anchor_comment.snapshot）を直接受け取らない" の記述で制御
# - final_classification 後の anchor_comment_feedback のみ渡すことが明記済み（AC6 で確認）
echo "PASS: no-raw-injection (AC6 で代替確認済み)"
