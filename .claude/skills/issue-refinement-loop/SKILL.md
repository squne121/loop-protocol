...
| `iteration >= max_iterations` かつ blocking ambiguity 残存 | `needs_second_pass` |
| `iteration >= max_iterations` かつ改善余地なし | `max_iterations` |
...
`max_iterations` デフォルト=1: Step 2 で `needs-fix` が返り `iteration >= max_iterations` に達したら `termination_reason: needs_second_pass` で停止。自動再ループ禁止。
## Guardrails
