...
1. `codebase-investigator` SubAgent で関連コードベース調査
...
本 skill は **control-plane**（state tracking + routing）のみ。本文編集は `issue-author` SubAgent が `edit-issue` skill 経由で行う。

| SubAgent / skill | 役割 |
|---|---|
| `codebase-investigator` SubAgent | 関連コードベース調査（read-only） |
| `review-issue` skill | 本文品質の決定論的判定 |
| `issue-author` SubAgent + `edit-issue` skill | 本文の安全な書き戻し |
...
Issue 本文の書き戻しは必ず `edit-issue` skill 経由で `gh issue edit --body-file` を使う:

- `tmp/issue-<番号>-body.md` に修正後本文全体を書き出してから edit
- HEREDOC のエスケープ事故防止のため `wc -c` と `grep -Pn '\\(?:\"|\$)'` で事前チェック
- 詳細は `.claude/skills/edit-issue/SKILL.md` 参照
...
- `docs/dev/agent-skill-boundaries.md` — 全体責務境界
- [references/issue-ops-and-handoff-sidecar.md](references/issue-ops-and-handoff-sidecar.md) — GitHub ops 記録プロトコルの補助ドキュメント
