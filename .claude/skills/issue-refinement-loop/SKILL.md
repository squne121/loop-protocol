...
**B8: anchor comment による stale approval 無効化**: `anchor_comment.snapshot != null` の場合、以下の条件でフローを制御する:
...
anchor comment が以下のいずれかを含む場合、`review-issue` の既存 approve を無効化して `needs-fix` として扱う:
...
Step 4 inputs（anchor_comment が feedback_update_required / reframe_in_place の場合）:
  ...
  anchor_comment_feedback:
    ...
    title_update:
      required: true | false   # Issue タイトルの変更が必要か
...
title_update.required:
  true_if_any:
    - final_classification == reframe_in_place かつ Issue の Outcome が大きく変わる場合
    - final_classification == feedback_update_required かつ anchor_comment が明示的にタイトル変更を要求している場合
  false_if_any:
    - final_classification == superseded_by_decision（タイトル更新対象外。代替 Issue 起票で対応）
    - structural-only needs-fix（AC/VC 整形のみ、In Scope 追加なし）でタイトルに影響しない場合
    - AC/VC 番号付け・整形のみの変更の場合
```
