...
`repair_issue_contract.py` が既知の safe な deterministic repair（disposition: `auto_apply_safe`）を1件以上検出した場合、`run_refinement_preflight.py` は generic blocker を追加せず `STATUS: needs_fix` / `NEXT_ACTION: apply_deterministic_repair` を返す。この `NEXT_ACTION: apply_deterministic_repair`（`repair_action.disposition: auto_apply_safe`）は、`command_registry.py` の `ISSUE_REFINEMENT_COMMAND_REGISTRY_V1` に登録済みの controlled consumer `repair_action.apply`（Issue #2039 / PR #2202、2026-08-16 マージ済み）を `skill_runtime_exec.py` 経由で `uv run python3 scripts/agent-guards/skill_runtime_exec.py --command-id repair_action.apply --issue-number <N> --repo <owner/repo> --apply-repair-action <preflight_result_path>` として起動し実処理する（`<preflight_result_path>` は `preflight.run` が生成した対象 Issue 自身の `.claude/artifacts/issue-refinement-loop/<N>/` 配下の fresh artifact パスに限られ、他 Issue の artifact や任意の repo-relative path は拒否される。内部は `edit_issue_txn.py` 経由の controlled transaction のみで、raw `gh issue edit` は呼ばない）。`repair_action.apply` は `required_cwd: canonical_main_root` / `required_branch: default_branch` で canonical main root から実行するが、`ROOT_NO_WORKTREE_ALLOWED_COMMAND_IDS` には含まれないため、対象 Issue 番号を `LOOP_ISSUE_NUMBER` 環境変数に設定した上で、`select_issue_worktree()` の命名規約（ブランチ短縮名とパス basename の両方が `(worktree-)?issue-<N>-*` に一致する単一 worktree）に合致する既存 worktree が `.claude/worktrees/` 配下に存在することも同時に要求する（未作成なら `worktree_bootstrap_exec.py` による worktree bootstrap を先に実行する。canonical main root からの実行と active-issue-bound worktree の存在は独立した別要件であり両方を満たす必要がある）。戻り値の `mutation_outcome` は `applied` / `no_change` / `unknown` / `not_attempted` の4値で、`applied`（`repair_action.apply` 内部の1回限りの authoritative readback で確認済み）または `no_change` の場合は readback 済みの本文を前提に fresh preflight（`preflight.run` の再実行）を行い Step 2 review を再開する。`unknown`（readback 自体が `repair_action.apply` 内部で既に一度だけ試行済みのため）は `repair_action.apply` を盲目的に再試行せず、bounded な report を添えて停止し人間判断を仰ぐ。`not_attempted`（`failure_code` 付き。例: repair candidate 単独では別の既存 readiness gate を解消できない等）の場合も `repair_action.apply` を再試行せず、通常の Step 2 review → Step 4 rewrite サイクルへフォールバックする。disposition が `auto_apply_safe` 以外（`human_review_required` / `informational` / `invalid_payload`）の場合はこの consumer の対象外であり、引き続き fail-closed で `STATUS: blocked` 相当の human judgment route のままである。
