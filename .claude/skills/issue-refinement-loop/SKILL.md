...
`REFINEMENT_LOOP_PLAN_V1.decisions.investigation_policy.required == true` の場合のみ `codebase-investigator` を起動する。返却される構造化結果を受け取り、`final_classification` の確定責務は main thread が保持する。SubAgent は mutation してはいけない。local asset 調査は `agy_investigation_requirement: advisory|explicitly_required` を単一の semantic policy input とする exact `AGY_ADVISORY_INVOCATION_REQUEST_V1` を controller に渡す。既存 caller の optional `agy_advisory_native_fallback_allowed` はこの pre-controller ingress だけで requirement へ one-way map し、変換後に削除する（requirement と legacy が両方あれば fail-close）。controller の exact exit-0 `degraded/native_non_mutating_fallback` pairing だけが bounded native investigation を許可し、`explicitly_required`・pre-AGY・malformed/unknown result は fail-close である。`root_entry_router.py::capability_preflight_result()` / `workflow_start_entry.py` への新配線は行わない。
