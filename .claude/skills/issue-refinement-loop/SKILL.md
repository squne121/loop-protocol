...
- `canonical_step2_route: step_5_human_judgment_required`（`status: ok` + `compact_result.verdict: needs-fix` + `compact_result.next_action: human_judgment_required` の **exact な組合せのみ**に対応。既存の compact schema — `review-issue/SKILL.md` の「Producer I/O ownership」節 — で有効な組。OWNER PR #2391 レビュー P1-1: `verdict` を確認しない緩い一致（`approve` + `human_judgment_required` 等）は下記 `fail_closed_environment_or_integrity_failure` に分類する）→ Step 5 (human_review_required) へ。genuine semantic / owner ambiguity 用であり、operator 起因の readiness 状態を扱う上記 `step_5_operator_intervention_required` とは別の停止理由として扱う
