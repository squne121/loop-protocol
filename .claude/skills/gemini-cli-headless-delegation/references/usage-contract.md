...
| `provider` | `"agy"` 固定。 |
| `tool_profile` | `"no_tools"` または `"proposal_only"` のみ。 |
...
| `grounded_research` / `local_asset_research` / `github_research` | 使用禁止。`unsupported_provider_profile` で拒否。 |

`objective` / `instructions` / `output_sections` / `context_files` は、既存 caller 互換のため指定されていてもよいが、
`provider=agy` 実行時の primary contract には含めない。caller は agy 専用 request を作るとき、
`prompt` と `tool_profile` を正本として扱う。
...
### Verification Status と Caller 責務（Fail-Closed）
...
### Caller-Side Re-Validation Rule
...
**Wrapper 内で file evidence が取得できた場合** → `delegation_result_v1.response_text` に REPO_EVIDENCE_REF_V1 の YAML / JSON を embed（またはファイル artifact）
