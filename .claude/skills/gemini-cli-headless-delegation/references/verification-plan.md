...
- `uv run --no-project --with pytest python3 -m pytest .claude/skills/gemini-cli-headless-delegation/tests -q` で 全テスト pass
...
- Windows（PowerShell）または WSL2 上の Python で `run_gemini_headless.py` を直接実行する:
  ```bash
  uv run --no-project python3 .claude/skills/gemini-cli-headless-delegation/scripts/run_gemini_headless.py \
