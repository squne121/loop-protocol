...
| `local_asset_research` | supported | B 案: wrapper 側で repo-boundary / payload / Serena contract を検証し、repo-relative JSON evidence envelope だけを prompt-only で AGY に渡す。 |
...
`local_asset_research` の AGY prompt は raw repo dump ではなく、次の provenance を持つ JSON evidence envelope を渡す。
