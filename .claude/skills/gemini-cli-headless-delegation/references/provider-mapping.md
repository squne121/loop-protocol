...
| `local_asset_research` | supported | B 案: wrapper 側で Serena 取得を実行し、prompt-only で AGY を起動。 |
...
| repo root 使用 | wrapper-only。`local_asset_research` では wrapper が repo root で証跡収集を行い、agy 側には渡さない |
