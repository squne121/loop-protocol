...
# タイトル更新が必要な場合はバックアップ時点の old_title も保存する（rollback 用）
OLD_TITLE=$(gh issue view "$ISSUE_NUMBER" --json title --jq '.title')
```
...
- `anchor_comment_feedback.title_update.required == true` の場合、`OLD_TITLE`（上記 `gh issue view --json title` で取得）を rollback 対象として保持する。
- Step 7（abort 時復旧）では body バックアップと共に `--title "$OLD_TITLE"` を使って元のタイトルに戻す。
...
`anchor_comment_feedback.title_update.required == true` の場合、`--title` オプションを同時に指定して body とタイトルを 1 コマンドで更新する。

```bash
# title_update.required == false（通常ケース）: body のみ更新
gh issue edit "$ISSUE_NUMBER" --repo "$REPO" --body-file "$NEW_BODY"
EDIT_EXIT=$?

# title_update.required == true: --title と --body-file を同時に指定して 1 コマンドで更新
# PROPOSED_TITLE は anchor_comment_feedback.title_update.proposed_title から設定する
gh issue edit "$ISSUE_NUMBER" --repo "$REPO" --title "$PROPOSED_TITLE" --body-file "$NEW_BODY"
EDIT_EXIT=$?
...
```bash
# title_update.required == false の場合: body のみ復旧
...
# title_update.required == true の場合: old_title と body を同時に復旧
# OLD_TITLE は Step 1 で保存した元タイトル
gh issue edit "$ISSUE_NUMBER" --repo "$REPO" --title "$OLD_TITLE" --body-file "$BACKUP_FILE" 2>/dev/null \
  || echo "[CRITICAL] バックアップ復旧失敗。手動対応必要。バックアップ: $BACKUP_FILE / old_title: $OLD_TITLE" >&2
