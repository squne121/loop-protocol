...
- `review-issue` skill の `needs-fix` 結果を Issue 本文に反映するとき
- `issue-refinement-loop` のステップ 4 で改訂された本文を書き戻すとき
- `post-merge-cleanup` の follow-up で既存 Issue へ追記が必要なとき
- 人間判断（`state/needs-human`）が解消され、本文へ反映するとき
