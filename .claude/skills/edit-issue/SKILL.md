...
BACKUP_FILE=$(python3 -c "import json,sys; print(json.loads(sys.argv[1])['backup_file'])" "$BACKUP_JSON")
# scripts entrypoint が title,body を同時に取得するため、OLD_TITLE も scripts 出力から取得する（rollback 用）
OLD_TITLE=$(python3 -c "import json,sys; print(json.loads(sys.argv[1])['title'])" "$BACKUP_JSON")
```

`backup-and-parse-issue.py` は `gh issue view <N> --repo <repo> --json title,body` を subprocess 配列形式で実行し、リポジトリルート配下の `tmp/issue_<N>_backup_<ts>.md` に body を保存したうえで `{"issue_number": <N>, "backup_file": "<path>", "title": "<title>", "body": "<body>"}` を stdout に返す。このバックアップは後続の全ステップで abort 時の復旧に使う。

**title の backup / rollback**:
- `OLD_TITLE` は `backup-and-parse-issue.py` の出力 JSON から取得する（inline `gh issue view` は使用しない — Guardrails 参照）。
...
`anchor_comment_feedback.title_update.required == true` の場合、`--title` オプションを同時に指定して body とタイトルを 1 コマンド（single command / co-update）で更新する。`title_update.required` の値に応じて排他的に分岐し、body-only 更新と title+body 更新が両方実行されることがないようにする。

```bash
# rollback 共通関数（Step 5 失敗時・Step 7 の双方で使用）
rollback_issue_edit() {
...
# TITLE_UPDATE_REQUIRED は anchor_comment_feedback.title_update.required から設定する（true / false）
# PROPOSED_TITLE は anchor_comment_feedback.title_update.proposed_title から設定する（required=true 時のみ）
if [ "${TITLE_UPDATE_REQUIRED:-false}" = "true" ]; then
...
ステップ 4, 5 のいずれかで fail した場合（Step 5 で定義した `rollback_issue_edit` 共通関数を使用する）:

```bash
# rollback_issue_edit は Step 5 で定義済み。
...
# OLD_TITLE は Step 1 で backup-and-parse-issue.py から取得した元タイトル
rollback_issue_edit
...
ISSUE_EDIT_RESULT_V1:
  ...
  title_update:  # optional（ISSUE_EDIT_RESULT_V1 の optional field）。anchor_comment_feedback.title_update.required == true の場合のみ設定する。タイトル更新が不要な場合（superseded_by_decision / structural-only needs-fix / AC/VC 整形のみ）は省略してよい。
    applied: true | false
    old_title: <変更前タイトル>
    new_title: <変更後タイトル>
  warnings: []
...
- **scripts entrypoint 経由統一**: バックアップ取得（title + body）・Guard 判定は必ず `.claude/skills/edit-issue/scripts/` のスクリプト経由で実行する。`OLD_TITLE` は `backup-and-parse-issue.py` の出力 JSON から取得すること（inline `gh issue view --json title` による title 単独取得は禁止）
