...
description: >
  session 記録 / Kill Switch / EntireCLI / checkpoint / secret-policy /
  agent-session-manifest 関連作業時に、Claude Code が手動呼び出しで参照できる操作手順 Skill。
...
session 記録 / Kill Switch / EntireCLI / checkpoint / secret-policy / agent-session-manifest
関連作業を行う際に、Claude Code が**手動呼び出しで**参照できる操作手順 Skill。

## 本 Skill の位置づけ

本 Skill は **policy 手順を提供するものであり、session 記録ツールの導入完了を意味しない**。
`disable-model-invocation: true` のため、自動発火 Skill ではなく**手動呼び出し専用**として扱う。
...
**失敗時**: policy check の出力を確認し、違反を修正する。

この Skill で `Bash` を使う場合は、上記 checker 実行や参照用の最小コマンドに限定し、
任意の変更系コマンド実行の根拠にしない。

### Step 3: SESSION_RECORDING_POLICY_VERDICT を記録する

以下の YAML を GitHub Issue コメント / PR 本文に記録する。
checker 実行結果に基づいて値を埋める。#377, #378, #379, #380 が main に merge されるまでは、
この VERDICT を session recording pilot / production enablement の証跡として扱ってはならない。

```yaml
SESSION_RECORDING_POLICY_VERDICT:
  generated_by: manual_attested_from_command_output
  checked_at: "YYYY-MM-DDTHH:MM:SSZ"
  repo: squne121/loop-protocol
  head_sha: "<commit-sha>"
  checker_command: "pnpm policy:check"
  checker_exit_code: 0
  checker: pass | fail
  ...
  evidence_refs:
    - "<CI run URL or redacted command output excerpt>"
