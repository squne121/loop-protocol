...
`--mode interactive` は常に（オプトインではなく）、isolated interactive lane 実行の前後で `herdr session list --json` と `herdr api snapshot` から呼び出し元自身の（デフォルト、非 isolated）Herdr session snapshot（session 識別・running/default 状態・socket path・session_dir に加え、workspace ID・agent ID・focus 中の workspace/tab/pane）を取得し、この run 自身が作成した isolated session を除いて完全に一致することを検証する（Issue #2174 AC7）。いずれかのフィールドが取得不能な場合も fail-closed で FAIL とする。evidence は `herdr_workspace_snapshot_diffs` / `herdr_workspace_snapshot_preserved` として記録される。
