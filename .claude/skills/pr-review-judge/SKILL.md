...
| **TEST_VERDICT_MACHINE の SKIP / fallback 検出（immediate のみ）** | linked issue の `decision: immediate` で、test-runner の `TEST_VERDICT_MACHINE` コメントに `verification_skipped_count: 0` かつ `runtime_ac_results` 内に `status: skip` / `fallback_detected: true` / `human_review_required: true` が存在しない | blocker（APPROVE 禁止）: exit 77 / `SKIP:` / `_*_fallback: true` の検出時は APPROVE しない |
