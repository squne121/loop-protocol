...
本 SubAgent が `gemini-cli-headless-delegation` 経由で返す file evidence は、`.claude/skills/gemini-cli-headless-delegation/references/usage-contract.md` の `REPO_EVIDENCE_REF_V1` スキーマに準拠する。本セクションは schema 本体を再定義せず、**consumer-side 受け取りルール** のみを定義する。

### Schema 再定義禁止

本 SubAgent 自身は `REPO_EVIDENCE_REF_V1` を拡張・変更しない。schema の唯一の正本は `gemini-cli-headless-delegation/references/usage-contract.md#REPO_EVIDENCE_REF_V1` である。

### Verification Status 処理ルール（fail-closed）
...
#### Rule 2: Verification Metadata 欠如による authoritative 扱い禁止

`REPO_EVIDENCE_REF_V1` に以下のフィールドが欠ける場合、evidence を「ソースコード事実」として信頼してはならない:

- `commit_sha` が存在しない / invalid（40-char SHA-1 または 64-char SHA-256 でない）
- `excerpt_sha256` が欠ける / invalid（64-char hex でない）
- `verification_status` / `verification_method` が明示されていない

この場合、本 SubAgent は以下を実行:

```python
required_fields = {'commit_sha', 'excerpt_sha256', 'verification_status', 'verification_method'}
if not all(evidence_ref.get(f) for f in required_fields):
    # → metadata が不完全 → evidence として use しない
    missing = [f for f in required_fields if not evidence_ref.get(f)]
    warnings.append(f"File evidence {path}#{start_line}-{end_line} lacks: {missing}")
    return {"status": "inconclusive", "reason": "verification_metadata_missing"}
```
