...
### source_evidence_result（dispositive source claim の acquisition failure 分類）(#2195)
...
- `SOURCE_EVIDENCE_ACQUISITION_RESULT_V1` の返却（`claim` / `baseline` / `route_plan` / `attempts` / `evidence_refs[]` / `semantic_verdict` / `disposition`）
