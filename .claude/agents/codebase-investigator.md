...
- `commit_sha` が存在しない / invalid（40-char SHA-1 でない）
- `excerpt_sha256` が欠ける
...
```python
if not evidence_ref.get('verification_status'):
    # → metadata が不完全 → evidence として use しない
    warnings.append(f"File evidence {path}#{start_line}-{end_line} lacks verification metadata")
