...
**VC 作成ガイダンス（`.claude/skills/issue-body-authoring/SKILL.md` 参照）**:
- `関数スコープ確認`: TypeScript の場合は `pnpm test <specific-test>` で実行系に委ねるのが最も決定論的。grep で関数スコープを限定したい場合は関数境界の正規表現で抽出する
...
- `.claude/skills/issue-body-authoring/SKILL.md` — VC 作成ガイダンス共通参照
