...
**identity / run binding**: `schema`、Issue/PR 番号、3種の HEAD、contract body SHA、run ID、run URL は省略不可。`pr_review_only` を含む adjudication では、全対象 AC の `command_hash`、`status: pass`、`exit_code: 0`、`fallback_detected: false`、`human_review_required: false`、`stop_condition_triggered: false` を report し、skip routing record 自体を実行済み証跡にしてはならない。
